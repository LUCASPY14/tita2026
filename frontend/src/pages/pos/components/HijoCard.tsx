...
const HijoCard: React.FC<HijoCardProps> = ({
  ...
  const getSaldoColor = () => {
    ...
    if (saldo.actual < 10000) return 'text-yellow-600 bg-yellow-50';
          ...
          <div className={`mt-3 inline-flex items-center gap-2 px-3 py-2 rounded-lg ${getSaldoColor()}`}>
            ...
            <div>
              <p className="text-sm font-medium">
                Saldo: {formatCurrency(saldo.actual)}
              </p>
              {saldo.alertaBajo && (
                <p className="text-xs">Saldo bajo - Recarga recomendada</p>
              )}
              {saldo.disponible > saldo.actual && (
                <p className="text-xs opacity-75">
                  Crédito disponible: {formatCurrency(saldo.disponible - saldo.actual)}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            {saldo.disponible > saldo.actual && (
              <div>
                ...
                <p className="font-semibold text-blue-600">{formatCurrency(saldo.disponible - saldo.actual)}</p>
              </div>
            )}
