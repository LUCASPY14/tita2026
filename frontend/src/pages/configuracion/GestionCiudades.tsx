...
import { MapPin, Plus, Pencil, Trash2, X, Check, RefreshCw } from 'lucide-react';
import toast from 'react-hot-toast';
import { ciudadesService } from '../../services/clientes.service';
import type { Ciudad } from '../../types';

const GestionCiudades: React.FC = () => {
  ...
  const abrirEditar = (c: Ciudad) => { setNombre(c.nombre); setEditId(c.id_ciudad); setMostrarForm(true); };
  ...
  const guardar = async () => {
    ...
    try {
      if (editId !== null) {
        await ciudadesService.actualizar(editId, nombre.trim());
        toast.success('Ciudad actualizada');
      } else {
        await ciudadesService.crear(nombre.trim());
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Catálogo de ciudades para clientes y proveedores</p>
          </div>
        </div>
        {!mostrarForm && (
          <button
      ...
      {mostrarForm && (
        <div className="bg-white border border-indigo-200 rounded-xl p-5 shadow-sm">
          <h3 className="font-semibold text-gray-800 mb-3">
            {editId ? 'Editar ciudad' : 'Nueva ciudad'}
          </h3>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {ciudades.length === 0 ? (
              ...
              ciudades.map(c => (
                <tr key={c.id_ciudad} className="border-b last:border-0 hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-400">{c.id_ciudad}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <button
                        onClick={() => handleEliminar(c.id_ciudad)}
                        className={`p-1.5 rounded transition-colors ${
                          eliminandoId === c.id_ciudad
                            ? 'bg-red-100 text-red-700'
                            : 'hover:bg-red-100 text-red-500'
                        }`}
                        title={eliminandoId === c.id_ciudad ? 'Clic de nuevo para confirmar' : 'Eliminar'}
                      >
      ...
      <p className="text-xs text-gray-400">{ciudades.length} registro(s)</p>
    </div>
  );
};
