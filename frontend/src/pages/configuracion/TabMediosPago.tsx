...
export default function TabMediosPago({ onDelete }: { onDelete: (t: DeleteTarget) => void }) {
  ...
  const save = useCallback(async () => {
    ...
    try {
      if (editing) {
        await api.put(`/core/medios-pago/${editing.id_medio_pago}/`, form)
    ...
    { title: 'Estado', key: 'activo', render: (_, r) => <Badge color={r.activo ? 'green' : 'default'}>{r.activo ? 'Activo' : 'Inactivo'}</Badge> },
    {
      ...
      render: (_, r) => (
        <div className="flex items-center gap-1">
          ...
          <Button size="sm" variant="danger" onClick={() => onDelete({ url: `/core/medios-pago/${r.id_medio_pago}/`, label: r.descripcion, reloadFn: load })}>
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-1">
          <Table columns={columns} dataSource={items} rowKey="id_medio_pago" loading={loading} pageSize={20} />
        </div>
      </div>
