...
export default function TabCategorias({ onDelete }: { onDelete: (t: DeleteTarget) => void }) {
  ...
  const save = useCallback(async () => {
    ...
    try {
      if (editing) {
        await api.put(`/productos/categorias/${editing.id_categoria}/`, form)
    ...
    { title: 'Descripción', key: 'desc', render: (_, r) => <span className="text-sm text-slate-500">{r.descripcion || '—'}</span> },
    {
      ...
      render: (_, r) => (
        <div className="flex items-center gap-1">
          ...
          <Button size="sm" variant="danger" onClick={() => onDelete({ url: `/productos/categorias/${r.id_categoria}/`, label: r.nombre, reloadFn: load })}>
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-1">
          <Table columns={columns} dataSource={items} rowKey="id_categoria" loading={loading} pageSize={20} />
        </div>
      </div>
