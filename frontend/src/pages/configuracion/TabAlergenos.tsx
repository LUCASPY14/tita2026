...
export default function TabAlergenos({ onDelete }: { onDelete: (t: DeleteTarget) => void }) {
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      if (editing) {
        await api.put(`/almuerzos/alergenos/${editing.id_alergeno}/`, payload)
    ...
    { title: 'Estado', key: 'activo', width: 90, render: (_, r) => <Badge color={r.activo ? 'green' : 'default'}>{r.activo ? 'Activo' : 'Inactivo'}</Badge> },
    {
      ...
      render: (_, r) => (
        <div className="flex items-center gap-1">
          ...
          <Button size="sm" variant="danger" onClick={() => onDelete({ url: `/almuerzos/alergenos/${r.id_alergeno}/`, label: r.nombre, reloadFn: load })}>
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-1">
          <Table columns={columns} dataSource={items} rowKey="id_alergeno" loading={loading} pageSize={20} />
        </div>
      </div>
