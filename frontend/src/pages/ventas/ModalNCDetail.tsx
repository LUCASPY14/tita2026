...
export default function ModalNCDetail({ nc, canAnular, onClose, onAnulada }: Props) {
  const [anulando, setAnulando] = useState(false)

  async function handleAnular() {
    if (!nc) return
    const extraMsg = nc.detalles?.length
  ...
  return (
    <Modal
      open={!!nc}
      title={nc ? `Nota de Crédito #${nc.nro_nota_credito} — ${nc.cliente_nombre}` : ''}
      onCancel={onClose}
      width={560}
      footer={null}
    >
        ...
        <div>
          <div className="grid grid-cols-2 gap-3 mb-4">
            {[
              ...
              { label: 'Fecha', value: formatFecha(nc.fecha_emision) },
              { label: 'Venta origen', value: nc.venta_origen ? `#${nc.venta_origen}` : '—' },
            ].map(({ label, value }) => (
          ...
          {nc.motivo && <p className="text-sm text-slate-500 mb-3">Motivo: {nc.motivo}</p>}
