import api from './api';
import { clientesService, ClienteParams, ClienteData } from './clientes.service';
import { Cliente, TipoCliente, CuentaCorriente, PaginatedResponse } from '../types';
...
describe('Clientes Service', () => {
  ...
  describe('getClientes', () => {
    ...
    test('debe obtener clientes sin parámetros', async () => {
      ...
      expect(mockedApi.get).toHaveBeenCalledWith('/clientes/', { params: undefined });
  ...
  describe('getClienteById', () => {
    ...
    test('debe manejar cliente no encontrado', async () => {
      ...
      await expect(clientesService.getClienteById(999)).rejects.toThrow('Not Found');
    });
  });
  ...
  describe('crearCliente', () => {
    ...
    test('debe crear cliente nuevo', async () => {
      ...
      expect(mockedApi.post).toHaveBeenCalledWith('/clientes/', mockClienteData);
  ...
  describe('actualizarCliente', () => {
    ...
    test('debe manejar cliente no encontrado', async () => {
      ...
      await expect(clientesService.actualizarCliente(999, {})).rejects.toThrow('Not Found');
    });
  });
  ...
  describe('eliminarCliente', () => {
    ...
    test('debe manejar error al eliminar cliente con ventas asociadas', async () => {
      mockedApi.delete.mockRejectedValue(new Error('Cannot delete client with sales'));
  ...
  describe('toggleEstadoCliente', () => {
    ...
    test('debe activar cliente', async () => {
      mockedApi.patch.mockResolvedValue({ data: { ...mockCliente, activo: true } });

      const result = await clientesService.toggleEstadoCliente(1, true);

      expect(mockedApi.patch).toHaveBeenCalledWith('/clientes/1/', { activo: true });
    ...
    test('debe desactivar cliente', async () => {
      ...
      expect(mockedApi.patch).toHaveBeenCalledWith('/clientes/1/', { activo: false });
  ...
  describe('getCuentaCorriente', () => {
    ...
    test('debe obtener cuenta corriente de cliente', async () => {
      ...
      expect(mockedApi.get).toHaveBeenCalledWith('/clientes/1/cuenta_corriente/');
  ...
  describe('getTiposCliente', () => {
    const mockTipos: TipoCliente[] = [
      { id: 1, nombre: 'Mayorista', activo: true },
      { id: 2, nombre: 'Minorista', activo: true }
    ];
  ...
  describe('Integration scenarios', () => {
    test('flujo completo: crear cliente -> actualizar -> obtener cuenta corriente', async () => {
      ...
      const updated = await clientesService.actualizarCliente(10, { email: 'updated@test.com' });
      ...
      expect(cuenta.cliente_id).toBe(10);
    });
  });
});
