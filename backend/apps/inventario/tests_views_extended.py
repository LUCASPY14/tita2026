...
class ReporteMermasMensualTest(TestCase):
    ...
    def test_con_mes_sin_datos_retorna_200(self):
        request = self.factory.get(
            ...
            {'mes': '2025-01'}  # Far past month, no data
        )
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True):
            response = self._get_view()(request)
    ...
    def test_con_datos_reales_analiza_motivos(self):
        ...
        AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='producto vencido', estado='Aprobado'
        )
        AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='daño físico', estado='Aprobado'
        )
        AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='Sin especificar', estado='Aprobado'
        )
...
class ProductosMayorDesperdicioTest(TestCase):
    ...
    def test_retorna_con_parametros_personalizados(self):
        request = self.factory.get(
            '/api/v1/ajustes-inventario/productos_mayor_desperdicio/',
            {'limite': '5', 'periodo_dias': '30'}
        )
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True):
            response = self._get_view()(request)
        self.assertEqual(response.status_code, 200)
...
class AnalisisCausasMermaTest(TestCase):
    ...
    def test_con_periodo_personalizado(self):
        request = self.factory.get(
            '/api/v1/ajustes-inventario/analisis_causas_merma/',
            {'periodo_dias': '30'}
        )
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True):
            response = self._get_view()(request)
    ...
    def test_con_datos_clasifica_todos_los_motivos(self):
        ...
        from apps.inventario.models import AjustesInventario
        AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='producto vencido en bodega', estado='Aprobado'
        )
        AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='daño en transporte', estado='Aprobado'
        )
        AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='robo confirmado', estado='Aprobado'
        )
        AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='conteo diferente al inventario real', estado='Aprobado'
        )
        AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='causa desconocida', estado='Aprobado'
        )
        request = self.factory.get('/api/v1/ajustes-inventario/analisis_causas_merma/')
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True):
            response = self._get_view()(request)
    ...
    def test_ajuste_sin_motivo_usa_sin_especificar(self):
        ...
        from apps.inventario.models import AjustesInventario
        AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='', estado='Aprobado'
        )
        request = self.factory.get('/api/v1/ajustes-inventario/analisis_causas_merma/')
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True):
            response = self._get_view()(request)
...
class PrediccionDemandaTest(TestCase):
    ...
    def test_producto_no_encontrado_retorna_404(self):
        ...
        request = self.factory.get(
            '/api/v1/ajustes-inventario/prediccion-demanda/',
            {'id_producto': '99999'}
        )
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True), \
...
class PuntoReordenTest(TestCase):
    ...
    def test_producto_no_encontrado_retorna_404(self):
        from apps.productos.models import Productos
        request = self.factory.get(
            '/api/v1/ajustes-inventario/punto-reorden/',
            {'id_producto': '99999'}
        )
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True), \
             patch('apps.productos.models.Productos.objects.get', side_effect=Productos.DoesNotExist):
...
class DetectarAnomaliasTest(TestCase):
    ...
    def test_producto_no_encontrado_retorna_404(self):
        from apps.productos.models import Productos
        request = self.factory.get(
            '/api/v1/ajustes-inventario/detectar-anomalias/',
            {'id_producto': '99999'}
        )
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True), \
             patch('apps.productos.models.Productos.objects.get', side_effect=Productos.DoesNotExist):
...
class RecomendacionCompraTest(TestCase):
    ...
    def test_producto_no_encontrado_retorna_404(self):
        from apps.productos.models import Productos
        request = self.factory.get(
            '/api/v1/ajustes-inventario/recomendacion-compra/',
            {'id_producto': '99999'}
        )
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True), \
             patch('apps.productos.models.Productos.objects.get', side_effect=Productos.DoesNotExist):
    ...
    def test_recomendacion_exitosa_con_urgencia_alta(self):
        ...
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True), \
             patch('apps.productos.models.Productos.objects.get', return_value=mock_producto), \
             patch('apps.inventario.models.StockUnico.objects.get', return_value=mock_stock), \
             patch('apps.inventario.views.StockForecastingService.obtener_recomendacion_compra', return_value=mock_recomendacion):
            response = self._get_view()(request)
        self.assertEqual(response.status_code, 200)
...
class AnalisisCompletoTest(TestCase):
    ...
    def test_producto_no_encontrado_retorna_404(self):
        from apps.productos.models import Productos
        request = self.factory.get(
            '/api/v1/ajustes-inventario/analisis-completo/',
            {'id_producto': '99999'}
        )
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True), \
             patch('apps.productos.models.Productos.objects.get', side_effect=Productos.DoesNotExist):
    ...
    def test_analisis_completo_sin_stock(self):
        ...
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True), \
             patch('apps.productos.models.Productos.objects.get', return_value=mock_producto), \
             patch('apps.inventario.models.StockUnico.objects.get', side_effect=StockUnico.DoesNotExist), \
             patch('apps.inventario.views.StockForecastingService.calcular_estadisticas_basicas', return_value={'tendencia': 'sin_datos', 'demanda_promedio_diaria': Decimal('0')}), \
             patch('apps.inventario.views.StockForecastingService.predecir_demanda_simple', return_value=[]), \
             patch('apps.inventario.views.StockForecastingService.calcular_punto_reorden', return_value={'punto_reorden': Decimal('0')}), \
             patch('apps.inventario.views.StockForecastingService.detectar_anomalias', return_value=[]), \
             patch('apps.inventario.views.StockForecastingService.obtener_recomendacion_compra', return_value={'urgencia': 'revisar'}), \
             patch('apps.inventario.views.StockForecastingService.analizar_estacionalidad', return_value={'tiene_estacionalidad': False}):
            response = self._get_view()(request)
        self.assertEqual(response.status_code, 200)
    ...
    def test_analisis_completo_exitoso_con_predicciones(self):
        ...
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True), \
             patch('apps.productos.models.Productos.objects.get', return_value=mock_producto), \
             patch('apps.inventario.models.StockUnico.objects.get', return_value=mock_stock), \
             patch('apps.inventario.views.StockForecastingService.calcular_estadisticas_basicas', return_value={'tendencia': 'estable', 'demanda_promedio_diaria': Decimal('5')}), \
             patch('apps.inventario.views.StockForecastingService.predecir_demanda_simple', return_value=mock_predicciones), \
             patch('apps.inventario.views.StockForecastingService.calcular_punto_reorden', return_value={'punto_reorden': Decimal('20')}), \
             patch('apps.inventario.views.StockForecastingService.detectar_anomalias', return_value=[]), \
             patch('apps.inventario.views.StockForecastingService.obtener_recomendacion_compra', return_value={'urgencia': 'baja'}), \
             patch('apps.inventario.views.StockForecastingService.analizar_estacionalidad', return_value={'tiene_estacionalidad': True}):
            response = self._get_view()(request)
        self.assertEqual(response.status_code, 200)
...
class ReporteMermasMensualConDatosTest(TestCase):
    ...
    def test_con_detalles_ajuste_cubre_producto_loop(self):
        ...
        ajuste = AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='vencido test rm1', estado='Aprobado'
        )
    ...
    def test_motivo_repetido_cubre_elif_por_motivo(self):
        ...
        for i in range(4):
            AjustesInventario.objects.create(
                tipo_ajuste='Merma', motivo='motivo repetido rm2', estado='Aprobado'
            )
        response = self._make_request()
        self.assertEqual(response.status_code, 200)
        self.assertIn('por_motivo', response.data)
    ...
    def test_tendencia_aumentando(self):
        ...
        aj_prev = AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='test ant', estado='Aprobado'
        )
    ...
    def test_tendencia_disminuyendo(self):
        ...
        prev_date = datetime.datetime(prev_year, prev_month, 15)
        for _ in range(3):
            aj = AjustesInventario.objects.create(
                tipo_ajuste='Merma', motivo='disminuye', estado='Aprobado'
            )
    ...
    def test_tendencia_estable(self):
        ...
        prev_date = datetime.datetime(prev_year, prev_month, 15)
        aj = AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='estable mes ant', estado='Aprobado'
        )
        AjustesInventario.objects.filter(pk=aj.pk).update(fecha_hora=prev_date)
...
class ProductosMayorDesperdicioConDatosTest(TestCase):
    ...
    def test_con_detalles_cubre_ranking_loop(self):
        ...
        ajuste = AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='test desperdicio', estado='Aprobado'
        )
        DetallesAjuste.objects.create(
            id_ajuste=ajuste,
            id_producto=producto,
...
class AnalisisCausasMermaMotivosTest(TestCase):
    ...
    def test_primera_vez_categoria_crea_entrada(self):
        ...
        AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='causa nueva 347', estado='Aprobado'
        )
        request = self.factory.get('/api/v1/ajustes-inventario/analisis_causas_merma/')
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True):
            response = self._get_view()(request)
    ...
    def test_categoria_repetida_cubre_false_branch(self):
        ...
        AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='producto vencido primera vez', estado='Aprobado'
        )
        AjustesInventario.objects.create(
            tipo_ajuste='Merma', motivo='producto vencido segunda vez', estado='Aprobado'
        )
        request = self.factory.get('/api/v1/ajustes-inventario/analisis_causas_merma/')
        with patch(PERM_PATCH_AUTH, return_value=True), patch(PERM_PATCH_INV, return_value=True):
            response = self._get_view()(request)
