...
class StockUnicoTest(TestCase):
    ...
    def test_requiere_reposicion(self):
        ...
        stock = StockUnico.objects.create(
            cantidad=Decimal('30.000'),  # Menor que stock_minimo (50)
            id_producto=self.producto
        )
...
class MovimientosStockTest(TestCase):
    ...
    def setUp(self):
        ...
        self.impuesto = Impuestos.objects.create(nombre_impuesto='IVA', porcentaje=10, vigente_desde=timezone.now().date())
...
class StockServiceTest(TestCase):
    ...
    def test_validar_disponibilidad_permite_negativo(self):
        ...
        resultado = StockService.validar_disponibilidad(
            self.producto.id_producto,
            Decimal('200.000')  # Más que el stock actual
        )
    ...
    def test_validar_multiple_productos(self):
        ...
        items = [
            ...
            {'id_producto': producto2.id_producto, 'cantidad': Decimal('30.000')}  # No hay suficiente
        ]
