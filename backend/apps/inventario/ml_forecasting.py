...
class StockForecastingService:
    ...
    def analizar_estacionalidad(id_producto: int, dias: int = 90) -> Dict:
        ...
        if valores:
            ...
            dias_pico = [dia for dia, valor in patron_semanal.items() 
                        if valor > promedio_general * 1.2]
            dias_valle = [dia for dia, valor in patron_semanal.items() 
        ...
        return {
            ...
            'recomendacion': 'Ajustar pedidos según patrones detectados' if tiene_estacionalidad else 'Demanda estable'
        }
    
    @staticmethod
    def obtener_recomendacion_compra(id_producto: int, stock_actual: Decimal, 
                                     dias_cobertura_deseada: int = 14) -> Dict:
        """
