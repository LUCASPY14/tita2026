...
class AjustesInventarioViewSet(viewsets.ModelViewSet):
    ...
    def reporte_mermas_mensual(self, request):
        ...
        # Analizar detalles
        detalles = DetallesAjuste.objects.filter(
            id_ajuste__in=ajustes_merma
        )
        
        total_unidades = detalles.aggregate(
            total=Sum('cantidad_ajustada')
        )['total'] or Decimal('0')
        ...
        productos_merma = detalles.values('id_producto').annotate(
        ...
        for ajuste in ajustes_merma:
            ...
            if len(por_motivo[motivo]['ejemplos']) < 3:
                por_motivo[motivo]['ejemplos'].append({
                    ...
                    'fecha': ajuste.fecha_hora.strftime('%Y-%m-%d')
                })
    ...
    def analisis_causas_merma(self, request):
        ...
        for ajuste in ajustes:
            ...
            if categoria not in motivos_clasificados:
                motivos_clasificados[categoria] = {
                    'frecuencia': 0,
                    'ajustes': []
                }
    ...
    def prediccion_demanda(self, request):
        ...
        if not id_producto:
            return Response(
                {'error': 'Parámetro id_producto es requerido'},
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def analisis_completo(self, request):
        ...
        demanda_total_7dias = sum(
            p['demanda_predicha'] for p in predicciones
        ) if predicciones else Decimal('0')
