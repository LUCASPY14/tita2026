...
def validar_cantidad_no_negativa(cantidad):
    ...
    if cantidad_decimal < 0:
        raise ValidationError(
            f"La cantidad no puede ser negativa. Valor recibido: {cantidad}"
        )
...
def validar_stock_minimo_maximo(cantidad_minima, cantidad_maxima):
    ...
    if min_decimal < 0:
        raise ValidationError(
            f"El stock mínimo no puede ser negativo: {cantidad_minima}"
        )
    
    if max_decimal <= 0:
        raise ValidationError(
            f"El stock máximo debe ser mayor a cero: {cantidad_maxima}"
        )
...
def validar_punto_reorden(punto_reorden, stock_minimo, stock_maximo):
    ...
    if reorden <= 0:
        raise ValidationError(
            f"El punto de reorden debe ser positivo: {punto_reorden}"
        )
...
def validar_stock_disponible(id_producto, cantidad_solicitada):
    ...
    try:
        ...
        raise ValidationError(
            f"No hay registro de stock para el producto {id_producto}"
        )
...
def validar_referencia_movimiento(tipo_referencia, id_referencia):
    ...
    TIPOS_REFERENCIA = [
        ...
        'Traslado', 'Produccion', 'Merma', 'Inicial'
    ]
    ...
    if id_referencia is not None and id_referencia <= 0:
        raise ValidationError(
            f"El ID de referencia debe ser positivo: {id_referencia}"
        )
...
def validar_estado_ajuste(estado):
    ...
    if estado not in ESTADOS_VALIDOS:
        raise ValidationError(
            ...
            f"Valores permitidos: {', '.join(ESTADOS_VALIDOS)}"
        )
...
def validar_numero_lote(numero_lote):
    ...
    if len(lote) < 3:
        raise ValidationError(
            f"El número de lote debe tener al menos 3 caracteres: '{lote}'"
        )
...
def validar_cantidad_lote(cantidad_lote, cantidad_movimiento):
    ...
    if lote <= 0:
        raise ValidationError(
            f"La cantidad del lote debe ser positiva: {cantidad_lote}"
        )
...
def validar_dias_historico(dias):
    ...
    if dias_int > 365:
        raise ValidationError(
            f"El análisis no puede superar 365 días. Valor recibido: {dias}"
        )
...
def validar_umbral_confianza(umbral):
    ...
    if umbral_float < 0.50:
        raise ValidationError(
            f"El umbral de confianza no puede ser menor a 0.50: {umbral}"
        )
    
    if umbral_float >= 1.0:
        raise ValidationError(
            f"El umbral de confianza debe ser menor a 1.0: {umbral}"
        )
...
def validar_lead_time(lead_time_dias):
    ...
    if lead_time < 1:
        raise ValidationError(
            f"El lead time debe ser al menos 1 día: {lead_time_dias}"
        )
    
    if lead_time > 90:
        raise ValidationError(
            f"El lead time no puede superar 90 días: {lead_time_dias}"
        )
...
def validar_dias_cobertura(dias_cobertura):
    ...
    if dias < 7:
        raise ValidationError(
            f"La cobertura mínima es 7 días: {dias_cobertura}"
        )
    
    if dias > 60:
        raise ValidationError(
            f"La cobertura máxima es 60 días: {dias_cobertura}"
        )
...
def validar_costo_unitario(costo):
    ...
    if costo_decimal <= 0:
        raise ValidationError(
            f"El costo unitario debe ser mayor a cero: {costo}"
        )
...
def validar_umbral_alerta(umbral_cantidad, stock_minimo, stock_maximo):
    ...
    if umbral < 0:
        raise ValidationError(
            f"El umbral de alerta no puede ser negativo: {umbral_cantidad}"
        )
