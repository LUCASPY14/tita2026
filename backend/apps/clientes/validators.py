...
def validar_motivo_cambio_grado(motivo):
    ...
    """
    motivos_validos = ['Promoción', 'Repetición', 'Transferencia', 'Corrección', 'Otro', 
                       'Promocion', 'Repeticion', 'Correccion']  # Con y sin tilde
...
def validar_severidad_restriccion(severidad):
    ...
    if severidad not in severidades_validas:
        raise ValidationError(
            f"Severidad inválida. Debe ser una de: Baja, Media, Alta, Crítica"
        )
...
def validar_tipo_operacion_log(tipo_operacion):
    ...
    """
    tipos_validos = [
        'Lectura', 'Autorización', 'Autorizacion', 'Validación', 'Validacion', 
        'Rechazo', 'Otro'
    ]
