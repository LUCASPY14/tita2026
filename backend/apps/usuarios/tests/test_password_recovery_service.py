...
from apps.usuarios.models import (
    Empleados, Roles, TokensRecuperacion, SesionesActivas
)
...
class RequestPasswordRecoveryTest(PasswordRecoveryServiceTest):
    ...
    def test_solicitar_recuperacion_email_inexistente(self):
        ...
        resultado = PasswordRecoveryService.solicitar_recuperacion_empleado(
            email='noexiste@cantinatita.com',
            ip_address=self.ip_address
        )
        ...
        tokens = TokensRecuperacion.objects.filter(
            tipo='password_recovery'
        ).count()
    ...
    def test_solicitar_recuperacion_expiracion_2_horas(self):
        ...
        self.assertAlmostEqual(
            tiempo_expiracion.total_seconds(),
            2 * 3600,
            delta=60  # +/- 1 minuto
        )
...
class CleanupExpiredTokensTest(PasswordRecoveryServiceTest):
    ...
    def test_limpiar_tokens_expirados(self):
        ...
        self.assertFalse(
            TokensRecuperacion.objects.filter(token_hash='token_viejo').exists()
        )
        self.assertTrue(
            TokensRecuperacion.objects.filter(token_hash='token_reciente').exists()
        )
        self.assertTrue(
            TokensRecuperacion.objects.filter(token_hash='token_valido').exists()
        )
