...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.MIGRATE_HEADING(
            '=== Ejecutando Limpieza del Sistema de Usuarios ===\n'
        ))
            ...
            if not dry_run:
                ...
                if resultado_sesiones['success']:
                    ...
                    self.stdout.write(self.style.SUCCESS(
                        f'   ✓ {sesiones_cerradas} sesiones cerradas'
                    ))
                    ...
                    self.stdout.write(self.style.ERROR(
                        f'   ✗ Error: {resultado_sesiones["mensaje"]}'
                    ))
            else:
                ...
                expiradas = SesionesActivas.objects.filter(
                    activa=True,
                    fecha_inicio__lt=ahora - timedelta(hours=24)
                ).count()
                
                inactivas = SesionesActivas.objects.filter(
                    activa=True,
                    ultima_actividad__lt=ahora - timedelta(minutes=30)
                ).count()
        ...
        try:
            if not dry_run:
                ...
                if resultado_tokens['success']:
                    ...
                    self.stdout.write(self.style.SUCCESS(
                        f'   ✓ {tokens_eliminados} tokens eliminados'
                    ))
                    ...
                    self.stdout.write(self.style.ERROR(
                        f'   ✗ Error: {resultado_tokens["mensaje"]}'
                    ))
            else:
                ...
                self.stdout.write(self.style.WARNING(
                    f'   → Se eliminarían {tokens_a_eliminar} tokens\n'
                ))
        ...
        try:
            if not dry_run:
                ...
                self.stdout.write(self.style.SUCCESS(
                    f'   ✓ {cantidad} intentos de login eliminados'
                ))
                ...
                self.stdout.write(self.style.WARNING(
                    f'   → Se eliminarían {cantidad} intentos de login\n'
                ))
        ...
        try:
            if not dry_run:
                ...
                total_limpiados += cantidad
                self.stdout.write(self.style.SUCCESS(
                    f'   ✓ {cantidad} intentos 2FA eliminados\n'
                ))
                ...
                self.stdout.write(self.style.WARNING(
                    f'   → Se eliminarían {cantidad} intentos 2FA\n'
                ))
        ...
        if dry_run:
            ...
            if errores:
                self.stdout.write(self.style.WARNING(
                    f'Total de elementos limpiados: {total_limpiados}'
                ))
                self.stdout.write(self.style.ERROR(
