    ...
    def handle(self, *args, **options):
        ...
        if resultado['permisos_creados'] > 0:
            self.stdout.write(self.style.SUCCESS(
                f'   ✓ {resultado["permisos_creados"]} permisos creados'
            ))
        if resultado['permisos_existentes'] > 0:
            self.stdout.write(self.style.WARNING(
                f'   ⚠ {resultado["permisos_existentes"]} permisos ya existían'
            ))
        
        self.stdout.write(self.style.SUCCESS(
            f'   ✓ Total de permisos en el sistema: {resultado["total"]}\n'
        ))
        ...
        roles = [
            ...
            },
            {
                ...
                'permisos': [
                    'ventas.ver', 'ventas.crear', 'ventas.editar', 'ventas.ver_reportes',
            ...
            },
            {
                ...
                'permisos': [
                    'reportes.ventas', 'reportes.inventario', 'reportes.financieros',
                    'ventas.ver',
                    'compras.ver',
        ...
        for rol_data in roles:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(
                    f'   ✓ Rol creado: {rol_data["nombre_rol"]}'
                ))
                ...
                self.stdout.write(self.style.WARNING(
                    f'   ⚠ Rol ya existía: {rol_data["nombre_rol"]}'
                ))
            ...
            if Empleados.objects.filter(usuario='admin').exists():
                ...
                if resultado['success']:
                    ...
                    self.stdout.write(self.style.ERROR(
                        f'   ✗ Error al crear admin: {resultado["mensaje"]}\n'
                    ))
