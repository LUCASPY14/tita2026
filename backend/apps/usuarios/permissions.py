...
class Permisos(models.Model):
    ...
    codigo_permiso = models.CharField(max_length=100, unique=True, 
                                      help_text="Código único del permiso (ej: 'ventas.crear')")
    nombre = models.CharField(max_length=100, help_text="Nombre descriptivo del permiso")
    modulo = models.CharField(max_length=50, 
                             help_text="Módulo al que pertenece (ventas, inventario, usuarios, etc)")
...
class RolesPermisos(models.Model):
    ...
    asignado_por = models.ForeignKey(Empleados, on_delete=models.SET_NULL, 
...
class PermissionService:
    ...
    @staticmethod
    def empleado_tiene_algunos_permisos(empleado: Empleados, 
                                        codigos_permisos: List[str]) -> bool:
        """
    ...
    @staticmethod
    def empleado_tiene_todos_permisos(empleado: Empleados, 
                                     codigos_permisos: List[str]) -> bool:
        """
    ...
    def obtener_permisos_empleado(empleado: Empleados) -> List[str]:
        ...
        permisos = RolesPermisos.objects.filter(
            id_rol=empleado.id_rol,
            id_permiso__activo=True
        ).select_related('id_permiso').values_list('id_permiso__codigo_permiso', flat=True)
        
        return list(permisos)
    
    @staticmethod
    @transaction.atomic
    def asignar_permiso_a_rol(rol: Roles, codigo_permiso: str, 
                             asignado_por: Empleados) -> Dict:
        """
        ...
        try:
            ...
            if creado:
                ...
                return {
                    'success': False,
                    'mensaje': 'El rol ya tiene este permiso'
                }
                
        except Permisos.DoesNotExist:
            return {
                'success': False,
                'mensaje': f'Permiso {codigo_permiso} no existe'
            }
        except Exception as e:
            return {
                'success': False,
                'mensaje': f'Error al asignar permiso: {str(e)}'
            }
    ...
    def remover_permiso_de_rol(rol: Roles, codigo_permiso: str) -> Dict:
        ...
        try:
            ...
            if eliminados > 0:
                ...
                return {
                    'success': False,
                    'mensaje': 'El rol no tiene este permiso'
                }
                
        except Exception as e:
            return {
                'success': False,
                'mensaje': f'Error al remover permiso: {str(e)}'
            }
