...
class AuthenticationService:
    ...
    def validar_fortaleza_password(password: str) -> Tuple[bool, str]:
        ...
        if len(password) < AuthenticationService.MIN_PASSWORD_LENGTH:
            return (
                False,
                f"La contraseña debe tener al menos {AuthenticationService.MIN_PASSWORD_LENGTH} caracteres",
            )
    ...
    @staticmethod
    @transaction.atomic
    def login(usuario: str, password: str, ip_address: str, user_agent: str = None) -> Dict:
        """
        ...
        try:
            # Buscar empleado
            empleado = Empleados.objects.select_related("id_rol").filter(usuario=usuario).first()
            ...
            # Login exitoso
            AuthenticationService._registrar_intento_login(empleado, ip_address, True)
    ...
    def cambiar_password(
        ...
        try:
            ...
            return {"success": False, "mensaje": "Error al cambiar contraseña"}
    ...
    def crear_empleado(
        ...
        try:
            ...
            return {"success": False, "mensaje": "El rol especificado no existe"}
        except Exception as e:
            print(f"Error al crear empleado: {str(e)}")
            return {"success": False, "mensaje": f"Error al crear empleado: {str(e)}"}
