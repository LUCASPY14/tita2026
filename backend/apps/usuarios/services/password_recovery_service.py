...
class PasswordRecoveryService:
    ...
    def solicitar_recuperacion_empleado(email: str, ip_address: str) -> Dict:
        ...
        try:
            ...
            solicitudes_hoy = TokensVerificacion.objects.filter(
                id_empleado=empleado, tipo="password_reset", fecha_creacion__gte=hoy_inicio
            ).count()
            ...
            return {"success": False, "mensaje": "Error al procesar solicitud de recuperación"}
    ...
    def validar_token_recuperacion(token: str, tipo_usuario: str = "empleado") -> Dict:
        ...
        try:
            ...
            return {"valido": False, "mensaje": "Error al validar token"}
    ...
    def restablecer_password_con_token(token: str, nueva_password: str, ip_address: str) -> Dict:
        ...
        try:
            ...
            token_hash = PasswordRecoveryService._hash_token(token)
            TokensVerificacion.objects.filter(token=token_hash, tipo="password_reset").update(
                usado=True, fecha_uso=timezone.now(), ip_uso=ip_address
            )
            ...
            SesionesActivas.objects.filter(id_empleado=empleado, activa=True).update(
                activa=False, fecha_cierre=timezone.now()
            )
            ...
            return {"success": False, "mensaje": f"Error al restablecer contraseña: {str(e)}"}
    ...
    def solicitar_verificacion_email(empleado: Empleados, ip_address: str) -> Dict:
        ...
        try:
            ...
            TokensVerificacion.objects.filter(
                id_empleado=empleado, tipo="email_verification", usado=False
            ).update(usado=True)
            ...
            AuditoriaOperaciones.objects.create(
                ...
                tabla_afectada="TokensVerificacion",
                ip_origen=ip_address,
                datos_nuevos={"email": empleado.email, "timestamp": str(timezone.now())},
            )

            return {"success": True, "token": token, "mensaje": "Token de verificación generado"}
    ...
    def limpiar_tokens_expirados() -> Dict:
        ...
        try:
            ...
            TokensVerificacion.objects.filter(fecha_expiracion__lt=fecha_limite).delete()
            ...
            TokensRecuperacion.objects.filter(fecha_expiracion__lt=fecha_limite).delete()

            total = tokens_eliminados + tokens_recuperacion_eliminados

            return {"tokens_eliminados": total, "mensaje": f"{total} tokens expirados eliminados"}

        except Exception as e:
            print(f"Error al limpiar tokens: {str(e)}")
            return {"tokens_eliminados": 0, "mensaje": f"Error al limpiar tokens: {str(e)}"}
