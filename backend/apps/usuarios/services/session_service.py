...
class SessionService:
    ...
    @staticmethod
    @transaction.atomic
    def crear_sesion(empleado: Empleados, session_key: str, ip_address: str,
        ...
        try:
            ...
            if sesiones_activas >= SessionService.MAX_SESIONES_SIMULTANEAS:
                ...
                sesion_mas_antigua = SesionesActivas.objects.filter(
                    id_empleado=empleado,
                    activa=True
            ...
            sesion = SesionesActivas.objects.create(
                ...
                fecha_expiracion=timezone.now() + timedelta(hours=SessionService.TIEMPO_EXPIRACION_HORAS),
                activa=True
            )
            ...
            return {
                'success': False,
                'mensaje': f'Error al crear sesión: {str(e)}'
            }
    
    @staticmethod
    @transaction.atomic
    def renovar_sesion(empleado: Empleados, session_key_actual: str, 
                      nuevo_session_key: str, ip_address: str) -> Dict:
        """
        ...
        try:
            ...
            sesion_actual = SesionesActivas.objects.filter(
                id_empleado=empleado,
                session_key=session_key_actual[:255],
                activa=True
            ).first()
            ...
            if not sesion_actual:
                return {
                    'success': False,
                    'mensaje': 'Sesión no encontrada o ya expirada'
                }
            
            # Verificar si ha pasado suficiente tiempo desde la última renovación
            ultima_renovacion = RenovacionesSesion.objects.filter(
                session_key_anterior=session_key_actual[:255]
            ).order_by('-fecha_renovacion').first()
            
            if ultima_renovacion:
                tiempo_transcurrido = timezone.now() - ultima_renovacion.fecha_renovacion
                if tiempo_transcurrido < timedelta(minutes=SessionService.RENOVACION_MINIMA_MINUTOS):
                    return {
                        'success': False,
            ...
            return {
                'success': False,
                'mensaje': f'Error al renovar sesión: {str(e)}'
            }
    ...
    def actualizar_actividad_sesion(empleado: Empleados, session_key: str) -> Dict:
        ...
        try:
            ...
            return {
                'success': False,
                'mensaje': f'Error al actualizar actividad: {str(e)}'
            }
    ...
    def cerrar_sesion(empleado: Empleados, session_key: str, ip_address: str) -> Dict:
        ...
        try:
            ...
            return {
                'success': False,
                'mensaje': f'Error al cerrar sesión: {str(e)}'
            }
    
    @staticmethod
    @transaction.atomic
    def cerrar_todas_sesiones(empleado: Empleados, ip_address: str, 
                             excepto_session_key: str = None) -> Dict:
        """
        ...
        """
        try:
            sesiones = SesionesActivas.objects.filter(
                id_empleado=empleado,
                activa=True
            )
            ...
            return {
                'success': False,
                'mensaje': f'Error al cerrar sesiones: {str(e)}'
            }
    ...
    def detectar_acceso_inusual(empleado: Empleados, ip_address: str) -> Dict:
        ...
        # Verificar horario
        hora_actual = timezone.now().hour
        patrones = PatronesAcceso.objects.filter(
            id_empleado=empleado,
            es_habitual=True
        )
        ...
        if patrones.exists():
            ...
            if horarios_habituales:
                ...
                if diferencia > 4:  # Más de 4 horas de diferencia
                    razones.append(f"Horario inusual (normalmente accede alrededor de las {int(horario_promedio)}:00)")
            ...
            for patron in patrones:
                if patron.dias_semana_habituales:
                    ...
                    if dia_actual not in dias_habituales:
                        ...
                        razones.append(f"Día inusual (normalmente no accede los {dias_nombres[dia_actual]})")
                        break
    ...
    def limpiar_sesiones_expiradas() -> Dict:
        ...
        try:
            ...
            sesiones_inactivas = SesionesActivas.objects.filter(
                activa=True,
                fecha_ultima_actividad__lt=tiempo_limite_inactividad
            )
            
            num_inactivas = sesiones_inactivas.count()
            sesiones_inactivas.update(
                activa=False,
                fecha_cierre=timezone.now()
            )
