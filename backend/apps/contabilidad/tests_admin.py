...
class BaseContabilidadAdminTest(TestCase):
    ...
    def setUp(self):
        ...
        self.caja = Cajas.objects.create(nombre_caja="Caja Admin Test", ubicacion="Administración", estado=True)

        self.medio_pago = MediosPago.objects.create(
            nombre="Efectivo Admin", descripcion="Para tests admin", estado=True
        )
...
class CajasAdminTest(BaseContabilidadAdminTest):
    ...
    def test_cajas_admin_search_functionality(self):
        ...
        caja1 = Cajas.objects.create(nombre_caja="Caja Principal Norte", ubicacion="Planta Alta - Norte")
        caja2 = Cajas.objects.create(nombre_caja="Caja Secundaria Sur", ubicacion="Planta Baja - Sur")
    ...
    def test_cajas_admin_form_validation(self):
        ...
        # Datos válidos
        valid_data = {"nombre_caja": "Caja Form Test", "ubicacion": "Test Location", "estado": True}
        ...
        invalid_data = {"nombre_caja": "", "ubicacion": "Test Location", "estado": True}
...
class CierresCajaAdminTest(BaseContabilidadAdminTest):
    ...
    def test_cierres_caja_admin_list_display(self):
        ...
        expected_fields = ["fecha_hora_apertura", "id_caja", "id_empleado", "estado", "monto_inicial"]
    ...
    def test_cierres_caja_admin_date_hierarchy(self):
        ...
        cierre_hoy = CierresCaja.objects.create(
            fecha_hora_apertura=hoy, estado="abierto", id_caja=self.caja, id_empleado=self.empleado
        )
        ...
        # Filtrar por fecha
        cierres_hoy = CierresCaja.objects.filter(fecha_hora_apertura__date=hoy.date())
        cierres_ayer = CierresCaja.objects.filter(fecha_hora_apertura__date=ayer.date())
...
class MovimientosCajaAdminTest(BaseContabilidadAdminTest):
    ...
    def test_movimientos_caja_admin_list_display(self):
        ...
        expected_fields = ["fecha_movimiento", "tipo_movimiento", "monto", "monto_comision", "id_medio_pago"]
    ...
    def test_movimientos_caja_admin_search_description(self):
        ...
        almuerzo_results = MovimientosCaja.objects.filter(descripcion__icontains="almuerzo")
        recarga_results = MovimientosCaja.objects.filter(descripcion__icontains="recarga")
    ...
    def test_movimientos_caja_admin_bulk_actions(self):
        ...
        MovimientosCaja.objects.filter(id_movimiento__in=ids_movimientos).update(descripcion="Revisado en lote")

        # Verificar actualización
        movimientos_actualizados = MovimientosCaja.objects.filter(descripcion="Revisado en lote")
...
class TarifasComisionAdminTest(BaseContabilidadAdminTest):
    ...
    def test_tarifas_comision_admin_vigencia_validation(self):
        ...
        es_vigente = tarifa_actual.fecha_inicio_vigencia <= fecha_actual and (
            tarifa_actual.fecha_fin_vigencia is None or tarifa_actual.fecha_fin_vigencia >= fecha_actual
        )
    ...
    def test_tarifas_comision_admin_filter_by_medio_pago(self):
        ...
        otro_medio = MediosPago.objects.create(nombre="Tarjeta Débito", descripcion="Pagos con débito", estado=True)
        ...
        tarifa_debito = TarifasComision.objects.create(
            fecha_inicio_vigencia=timezone.now(), porcentaje_comision=Decimal("3.0000"), id_medio_pago=otro_medio
        )

        # Filtrar por medio de pago
        tarifas_efectivo = TarifasComision.objects.filter(id_medio_pago=self.medio_pago)
        tarifas_debito = TarifasComision.objects.filter(id_medio_pago=otro_medio)
...
class DocumentosTributariosAdminTest(BaseContabilidadAdminTest):
    ...
    def setUp(self):
        ...
        self.punto = PuntosExpedicion.objects.create(
            codigo_establecimiento="001", codigo_punto_expedicion="001", descripcion_ubicacion="Principal Admin"
        )
    ...
    def test_documentos_tributarios_admin_list_display(self):
        ...
        expected_fields = ["nro_secuencial", "fecha_emision", "monto_total", "tipo_documento", "nro_timbrado"]
    ...
    def test_documentos_tributarios_admin_date_hierarchy(self):
        ...
        # Filtrar por fecha
        docs_hoy = DocumentosTributarios.objects.filter(fecha_emision__date=hoy.date())
        docs_ayer = DocumentosTributarios.objects.filter(fecha_emision__date=ayer.date())
...
class ImpuestosAdminTest(BaseContabilidadAdminTest):
    ...
    def test_impuestos_admin_list_display(self):
        ...
        impuesto = Impuestos.objects.create(
            nombre_impuesto="IVA 10% Admin", porcentaje=Decimal("10.00"), vigente_desde=date.today(), estado=True
        )

        # Campos para mostrar
        expected_fields = ["nombre_impuesto", "porcentaje", "vigente_desde", "vigente_hasta", "estado"]
    ...
    def test_impuestos_admin_filter_by_activo(self):
        ...
        impuesto_activo = Impuestos.objects.create(
            nombre_impuesto="IVA estado", porcentaje=Decimal("10.00"), vigente_desde=date.today(), estado=True
        )
    ...
    def test_impuestos_admin_search_by_nombre(self):
        ...
        iva_10 = Impuestos.objects.create(
            nombre_impuesto="IVA 10% Standard", porcentaje=Decimal("10.00"), vigente_desde=date.today()
        )

        iva_5 = Impuestos.objects.create(
            nombre_impuesto="IVA 5% Reducido", porcentaje=Decimal("5.00"), vigente_desde=date.today()
        )
    ...
    def test_impuestos_admin_calculation_preview(self):
        ...
        impuesto = Impuestos.objects.create(
            nombre_impuesto="IVA Cálculo Test", porcentaje=Decimal("10.00"), vigente_desde=date.today()
        )

        # Preview para diferentes montos
        montos_test = [Decimal("100000.00"), Decimal("250000.00"), Decimal("500000.00")]
    ...
    def test_impuestos_admin_vigencia_validation(self):
        ...
        es_vigente = impuesto_valido.vigente_desde <= fecha_actual and (
            impuesto_valido.vigente_hasta is None or impuesto_valido.vigente_hasta >= fecha_actual
        )
        self.assertTrue(es_vigente)
...
class ContabilidadAdminIntegrationTest(BaseContabilidadAdminTest):
    ...
    def test_admin_bulk_operations(self):
        ...
        cajas_bulk = []
        for i in range(10):
            caja = Cajas.objects.create(nombre_caja=f"Caja Bulk {i}", ubicacion=f"Ubicación {i}", estado=True)
        ...
        cajas_desactivadas = Cajas.objects.filter(id_caja__in=cajas_ids, estado=False)
    ...
    def test_admin_security_validations(self):
        ...
        user_limited = User.objects.create_user(username="limited", password="limited123")
