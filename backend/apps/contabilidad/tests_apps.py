...
class ContabilidadAppsConfigTest(TestCase):
    ...
    def test_contabilidad_app_installed(self):
        ...
        app_installed = any("contabilidad" in app.lower() for app in installed_apps)

        # En un entorno de test, debería estar instalada
        self.assertTrue(
            app_installed or "apps.contabilidad" in installed_apps, "App contabilidad debe estar en INSTALLED_APPS"
        )
    ...
    def test_contabilidad_app_models_loaded(self):
        ...
        try:
            ...
            for expected_model in expected_models:
                self.assertIn(expected_model, model_names, f"Modelo {expected_model} debe estar definido")
...
class ContabilidadAppsIntegrationTest(TestCase):
    ...
    def test_contabilidad_app_migrations_exist(self):
        ...
        try:
            ...
            contabilidad_migrations = [migration for migration in migrations if migration[0] == "contabilidad"]

            # Si la app está configurada, debería tener al menos una migración
            if apps.is_installed("apps.contabilidad"):
                self.assertGreater(len(contabilidad_migrations), 0, "Debe tener migraciones definidas")
    ...
    def test_contabilidad_app_serializers_integration(self):
        ...
        try:
            ...
            serializer_classes = [
                attr for attr in dir(serializers) if attr.endswith("Serializer") and not attr.startswith("_")
            ]
            ...
            for serializer_name in serializer_classes:
                ...
                if hasattr(serializer_class, "__mro__"):
                    ...
                    is_drf_serializer = any("Serializer" in cls_name for cls_name in base_classes)

                    self.assertTrue(is_drf_serializer, f"{serializer_name} debe heredar de serializer DRF")
    ...
    def test_contabilidad_app_views_integration(self):
        ...
        try:
            ...
            view_classes = [
                ...
                if (attr.endswith("ViewSet") or attr.endswith("View")) and not attr.startswith("_")
            ]
            ...
            for view_name in view_classes:
                ...
                if hasattr(view_class, "__mro__"):
                    ...
                    if is_drf_view:
                        self.assertTrue(is_drf_view, f"{view_name} debe heredar de view/viewset DRF")
            ...
            if hasattr(urls, "urlpatterns"):
                ...
                for pattern in urlpatterns:
                    self.assertIsInstance(
                        pattern, (URLPattern, URLResolver), "Cada patrón debe ser URLPattern o URLResolver válido"
                    )
    ...
    def test_contabilidad_app_permissions_integration(self):
        ...
        try:
            ...
            # Si están definidos, verificar que son válidos
            permission_classes = [
                attr for attr in dir(permissions) if attr.endswith("Permission") and not attr.startswith("_")
            ]
            ...
            for permission_name in permission_classes:
                ...
                if hasattr(permission_class, "__mro__"):
                    ...
                    is_drf_permission = any("Permission" in cls_name for cls_name in base_classes)

                    if is_drf_permission:
                        self.assertTrue(is_drf_permission, f"{permission_name} debe heredar de permission DRF")
...
class ContabilidadAppsMetaTest(TestCase):
    ...
    def test_contabilidad_app_structure(self):
        ...
        optional_files = ["admin.py", "views.py", "urls.py", "serializers.py", "permissions.py", "signals.py"]

        for required_file in required_files:
            file_path = os.path.join(app_path, required_file)
            self.assertTrue(os.path.exists(file_path), f"Archivo requerido {required_file} debe existir")

        # Verificar que al menos algunos archivos opcionales existen
        existing_optional = [file for file in optional_files if os.path.exists(os.path.join(app_path, file))]

        self.assertGreater(len(existing_optional), 0, "Debe tener al menos algunos archivos opcionales de Django app")
    ...
    def test_contabilidad_app_naming_conventions(self):
        ...
        self.assertTrue(app_config.__name__.endswith("Config"), "Clase de configuración debe terminar en 'Config'")

        # Verificar nombre de app
        self.assertEqual(app_config.name, "apps.contabilidad", "Nombre de app debe seguir patrón 'apps.nombre'")
    ...
    def test_contabilidad_app_settings_integration(self):
        ...
        critical_settings = ["SECRET_KEY", "INSTALLED_APPS"]

        for setting_name in critical_settings:
            self.assertTrue(
                hasattr(settings, setting_name), f"Configuración crítica {setting_name} debe estar presente"
            )
