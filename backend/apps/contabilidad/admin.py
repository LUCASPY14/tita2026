...
class CierresCajaAdmin(admin.ModelAdmin):
    list_display = ['id_cierre', 'id_caja', 'fecha_hora_apertura', 'fecha_hora_cierre', 
                    'monto_inicial_display', 'monto_contado_display', 'diferencia_display', 'estado_badge']
    ...
    fieldsets = (
        ('Información del Cierre', {
            'fields': ('id_cierre', 'id_caja', 'id_empleado')
        }),
        ('Fechas', {
            'fields': ('fecha_hora_apertura', 'fecha_hora_cierre', 'duracion_display')
        }),
        ('Montos', {
    ...
    def estado_badge(self, obj):
        ...
        return format_html(
            ...
            color, obj.estado if obj.estado else 'N/A'
        )
...
class MovimientosCajaAdmin(admin.ModelAdmin):
    list_display = ['id_movimiento', 'id_cierre', 'tipo_movimiento_badge', 'monto_display', 
                    'monto_comision_display', 'fecha_movimiento']
    ...
    fieldsets = (
        ...
        ('Montos', {
            'fields': ('monto', 'monto_comision')
        }),
        ('Detalles', {
            'fields': ('fecha_movimiento', 'descripcion', 'id_venta')
        }),
    )
...
class TarifasComisionAdmin(admin.ModelAdmin):
    list_display = ['id_tarifa', 'id_medio_pago', 'porcentaje_display', 'monto_fijo_display',
                    'fecha_inicio_vigencia', 'fecha_fin_vigencia', 'activo_badge']
    ...
    fieldsets = (
        ('Información General', {
            'fields': ('id_tarifa', 'id_medio_pago')
        }),
        ('Comisiones', {
    ...
    def porcentaje_display(self, obj):
        return format_html('<strong>{:.2f}%</strong>', obj.porcentaje_comision * 100 if obj.porcentaje_comision else 0)
...
class AuditoriaComisionesAdmin(admin.ModelAdmin):
    list_display = ['id_auditoria', 'id_tarifa', 'campo_modificado', 'valor_anterior_display',
                    'valor_nuevo_display', 'id_empleado_modifico', 'fecha_cambio']
    list_filter = ['campo_modificado', 'fecha_cambio']
    search_fields = ['campo_modificado']
    ordering = ['-fecha_cambio']
    readonly_fields = ['id_auditoria', 'fecha_cambio', 'id_tarifa', 'campo_modificado',
                       'valor_anterior', 'valor_nuevo', 'id_empleado_modifico']
    ...
    def valor_nuevo_display(self, obj):
        if obj.valor_nuevo is not None:
            return format_html('<span style="color: green; font-weight: bold;">{:.4f}</span>', obj.valor_nuevo)
        return '-'
...
class ConciliacionPagosAdmin(admin.ModelAdmin):
    list_display = ['id_conciliacion', 'id_pago_venta', 'estado_badge', 'fecha_conciliacion',
                    'fecha_acreditacion', 'monto_acreditado_display']
    ...
    fieldsets = (
        ('Información General', {
            'fields': ('id_conciliacion', 'id_pago_venta', 'estado')
        }),
        ('Fechas', {
            'fields': ('fecha_conciliacion', 'fecha_acreditacion', 'fecha_creacion', 'fecha_actualizacion')
        }),
        ('Montos', {
            'fields': ('monto_acreditado',)
        }),
        ('Observaciones', {
...
class DocumentosTributariosAdmin(admin.ModelAdmin):
    list_display = ['id_documento', 'nro_secuencial', 'tipo_documento_badge', 'monto_total_display',
                    'fecha_emision', 'estado_sifen_badge', 'nro_timbrado']
    ...
    fieldsets = (
        ...
        ('Documentación Electrónica', {
            'fields': ('cdc', 'url_kude', 'estado_sifen')
        }),
        ('Fechas', {
            'fields': ('fecha_emision', 'fecha_envio', 'fecha_respuesta')
        }),
        ('Otros', {
            'fields': ('nro_preimpreso_interno',)
        }),
    )
...
class TimbradosAdmin(admin.ModelAdmin):
    list_display = ['nro_timbrado', 'tipo_documento', 'fecha_inicio', 'fecha_fin',
                    'numeros_display', 'es_electronico_badge', 'activo_badge']
    ...
    fieldsets = (
        ...
        ('Numeración', {
            'fields': ('nro_inicial', 'nro_final', 'disponibles_display')
        }),
        ('Estado', {
            'fields': ('activo',)
        }),
    ...
    def es_electronico_badge(self, obj):
        if obj.es_electronico:
            return format_html('<span style="background-color: blue; color: white; padding: 3px 8px; border-radius: 3px;">📱 Digital</span>')
        return format_html('<span style="background-color: gray; color: white; padding: 3px 8px; border-radius: 3px;">📄 Papel</span>')
...
class PuntosExpedicionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Información del Punto', {
            'fields': ('id_punto', 'codigo_establecimiento', 'codigo_punto_expedicion', 'codigo_completo_display')
        }),
        ('Detalles', {
            'fields': ('descripcion_ubicacion',)
        }),
        ('Estado', {
            'fields': ('activo',)
        }),
...
class DatosEmpresaAdmin(admin.ModelAdmin):
    list_display = ['id_empresa', 'razon_social', 'ruc', 'ciudad', 'telefono', 'email', 'activo_badge']
    ...
    fieldsets = (
        ('Información Fiscal', {
            'fields': ('id_empresa', 'ruc', 'razon_social')
        }),
        ('Ubicación', {
...
class ImpuestosAdmin(admin.ModelAdmin):
    list_display = ['id_impuesto', 'nombre_impuesto', 'porcentaje_display', 'vigente_desde',
                    'vigente_hasta', 'activo_badge']
    ...
    fieldsets = (
        ('Información del Impuesto', {
            'fields': ('id_impuesto', 'nombre_impuesto', 'porcentaje')
        }),
        ('Vigencia', {
