...
class PlantillasReporteSerializer(serializers.ModelSerializer):
    ...
    def validate_nombre(self, value):
        ...
        query = PlantillasReporte.objects.filter(nombre=value.strip(), estado=True)
        ...
        if query.exists():
            raise serializers.ValidationError("Ya existe una plantilla activa con este nombre")
        ...
        if tipo_reporte in campos_requeridos:
            for campo in campos_requeridos[tipo_reporte]:
                if campo not in configuracion:
                    raise serializers.ValidationError(f"Campo '{campo}' requerido para reporte tipo '{tipo_reporte}'")
...
class DashboardsSerializer(serializers.ModelSerializer):
    ...
    orden_widgets = serializers.ListField(
        child=serializers.IntegerField(), required=False, help_text="Lista de IDs de widgets en orden de visualización"
    )
    ...
    def validate_configuracion_dashboard(self, value):
        ...
        for campo in campos_requeridos:
            if campo not in value:
                raise serializers.ValidationError(f"Campo '{campo}' requerido en configuración")
    ...
    def validate_nombre(self, value):
        ...
        if query.exists():
            raise serializers.ValidationError("Ya existe un dashboard estado con este nombre")
...
class KpiMetricasSerializer(serializers.ModelSerializer):
    ...
    # Validaciones
    orden_visualizacion = serializers.IntegerField(validators=[MinValueValidator(1), MaxValueValidator(100)])
    ...
    def validate_configuracion_calculo(self, value):
        ...
        for campo in campos_base:
            if campo not in value:
                raise serializers.ValidationError(f"Campo '{campo}' requerido en configuración de cálculo")

        # Validar fórmula
        formula = value.get("formula", "")
        if not formula or len(formula.strip()) < 3:
            raise serializers.ValidationError("La fórmula debe tener al menos 3 caracteres")
    ...
    def validate_tipo_metrica(self, value):
        ...
        if value not in tipos_validos:
            raise serializers.ValidationError(f"Tipo de métrica inválido. Opciones: {', '.join(tipos_validos)}")
        return value
...
class ValoresKpiSerializer(serializers.ModelSerializer):
    ...
    valor = serializers.DecimalField(
        max_digits=15, decimal_places=4, validators=[MinValueValidator(Decimal("-999999999.9999"))]
    )

    class Meta:
        model = ValoresKpi
        fields = ["id_valor", "id_kpi", "valor", "unidad_medida", "fecha_calculo", "observaciones", "kpi_nombre"]
    ...
    def validate_valor(self, value):
        ...
        if value < Decimal("-999999999") or value > Decimal("999999999"):
            raise serializers.ValidationError("El valor está fuera del rango válido")
...
class PlantillasTareaSerializer(serializers.ModelSerializer):
    ...
    frecuencia_ejecucion = serializers.ChoiceField(
        choices=[("diaria", "Diaria"), ("semanal", "Semanal"), ("mensual", "Mensual"), ("manual", "Manual")]
    )
    ...
    def get_proxima_ejecucion(self, obj):
        ...
        ultima_ejecucion = obj.ejecuciones.filter(estado="completada").order_by("-fecha_ejecucion").first()
...
class EjecucionesTareaSerializer(serializers.ModelSerializer):
    ...
    # Campos relacionados
    plantilla_nombre = serializers.CharField(source="id_plantilla_tarea.nombre_tarea", read_only=True)
    empleado_ejecutor = EmpleadoBasicoSerializer(source="id_empleado_ejecutor", read_only=True)
    ...
    class Meta:
        ...
        read_only_fields = ["id_ejecucion", "fecha_ejecucion", "fecha_finalizacion"]
...
class DestinatariosTareaSerializer(serializers.ModelSerializer):
    ...
    def validate_tipo_notificacion(self, value):
        ...
        if value not in tipos_validos:
            raise serializers.ValidationError(f"Tipo de notificación inválido. Opciones: {', '.join(tipos_validos)}")
        return value
...
class ReporteVentasRequestSerializer(serializers.Serializer):
    ...
    fecha_inicio = serializers.DateField(help_text="Fecha de inicio del período (formato: YYYY-MM-DD)")
    fecha_fin = serializers.DateField(help_text="Fecha de fin del período (formato: YYYY-MM-DD)")
    id_empleado = serializers.IntegerField(required=False, help_text="ID del empleado para filtrar (opcional)")
    metodo_pago = serializers.ChoiceField(
        choices=[("efectivo", "Efectivo"), ("tarjeta", "Tarjeta"), ("online", "Online")],
        required=False,
        help_text="Método de pago para filtrar (opcional)",
    )
    incluir_detalles = serializers.BooleanField(default=True, help_text="Incluir detalles de productos vendidos")
    limite_productos = serializers.IntegerField(
        default=10, min_value=1, max_value=100, help_text="Límite de productos en top productos"
    )
    ...
    def validate(self, data):
        ...
        if fecha_inicio and fecha_fin:
            ...
            if diferencia > 365:
                raise serializers.ValidationError("El rango de fechas no puede exceder 365 días")
...
class ReporteFinancieroRequestSerializer(serializers.Serializer):
    ...
    desglosar_por = serializers.ChoiceField(
        choices=[("dia", "Día"), ("semana", "Semana"), ("mes", "Mes")], default="dia"
    )
    moneda = serializers.ChoiceField(choices=[("COP", "Pesos Colombianos"), ("USD", "Dólares")], default="COP")
...
class DashboardRequestSerializer(serializers.Serializer):
    ...
    periodo_dias = serializers.IntegerField(
        default=7, min_value=1, max_value=365, help_text="Período en días para el dashboard"
    )
    incluir_comparacion = serializers.BooleanField(default=True, help_text="Incluir comparación con período anterior")
