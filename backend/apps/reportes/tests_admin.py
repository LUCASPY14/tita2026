...
class BaseReportesAdminTest(TestCase):
    ...
    def setUp(self):
        ...
        self.admin_user = User.objects.create_superuser(
            username="admin_reportes", email="admin@reportes.com", password="admin123"
        )
        ...
        self.rol = Roles.objects.create(
            nombre_rol="Analista Reportes", descripcion="Rol de analista de reportes", estado=True
        )
            ...
            "widgets": [
                {
                    ...
                    "configuracion": {"colores": ["#FF6384", "#36A2EB"], "eje_y": {"min": 0, "formato": "currency"}},
...
class ReportesAdminIntegrationTest(BaseReportesAdminTest):
    ...
    def test_admin_workflow_complete_report_creation(self):
        ...
        tarea_config = {"tipo": "scheduled", "frecuencia": "daily", "hora": "08:00"}
    ...
    def test_admin_performance_considerations(self):
        ...
        page_size = 10
        first_page = PlantillasReporte.objects.filter(tipo_reporte="performance_test")[:page_size]

        self.assertEqual(len(first_page), page_size)

        # Simular optimización con select_related
        plantillas_optimized = PlantillasReporte.objects.select_related("created_by").filter(
            tipo_reporte="performance_test"
        )[:5]
