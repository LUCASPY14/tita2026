...
def validar_forma_cobro(valor):
    ...
    formas_validas = ['Efectivo', 'Transferencia', 'Tarjeta', 'Cuenta Corriente', 'Débito Automático']
...
def validar_coherencia_montos_cuenta(monto_total, monto_pagado):
    ...
    if monto_pagado > tolerancia:
        raise ValidationError(f'El monto pagado (₲{monto_pagado:,.2f}) excede el monto total + tolerancia (₲{tolerancia:,.2f}).')
