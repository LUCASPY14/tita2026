...
class TestSuscripcionesAlmuerzoMetodos:
    ...
    def hijo(self):
        ...
        cliente = Clientes.objects.create(
            nombres="Fernando", apellidos="Silva", ruc_ci="3333333", estado=True, id_lista=lista, id_tipo_cliente=tipo
        )
...
class TestRegistrosConsumoAlmuerzoMetodos:
    ...
    def hijo(self):
        ...
        tipo = TiposCliente.objects.create(nombre_tipo="Padre", estado=True)
        cliente = Clientes.objects.create(
            nombres="Patricia", apellidos="Gómez", ruc_ci="4444444", estado=True, id_lista=lista, id_tipo_cliente=tipo
        )
        return Hijos.objects.create(
            nombre="Mateo",
            apellido="Gómez",
...
class TestCuentasAlmuerzoMensualMetodos:
    ...
    def hijo(self):
        ...
        tipo = TiposCliente.objects.create(nombre_tipo="Padre", estado=True)
        cliente = Clientes.objects.create(
            nombres="Ricardo", apellidos="Benítez", ruc_ci="5555555", estado=True, id_lista=lista, id_tipo_cliente=tipo
        )
        return Hijos.objects.create(
            nombre="Isabella",
            apellido="Benítez",
...
class TestAlmuerzosModelsIntegracion:
    ...
    def test_flujo_completo_suscripcion_a_consumo(self):
        ...
        tipo = TiposCliente.objects.create(nombre_tipo="Padre", estado=True)
        cliente = Clientes.objects.create(
            nombres="Miguel", apellidos="Rojas", ruc_ci="6666666", estado=True, id_lista=lista, id_tipo_cliente=tipo
        )
        ...
        plan = PlanesAlmuerzo.objects.create(nombre_plan="Plan Básico", precio_mensual=Decimal("400.00"), estado=True)

        # Act: Crear todos los objetos relacionados
        suscripcion = SuscripcionesAlmuerzo.objects.create(
            id_hijo=hijo, id_plan_almuerzo=plan, fecha_inicio=date.today(), estado=True
        )

        registro = RegistrosConsumoAlmuerzo.objects.create(
            id_hijo=hijo, fecha_consumo=date.today(), hora_registro=time(12, 0), estado="registrado", ya_cobrado=True
        )
