...
class TestRegistrosConsumoAlmuerzoViewSetPerformCreate:
    ...
    def user(self):
        ...
        return User.objects.create_user(
            username="testuser", password="testpass123", is_staff=True  # Staff para bypasear permissions
        )
    ...
    def hijo(self):
        ...
        cliente = Clientes.objects.create(
            nombres="Carlos", apellidos="Martínez", ruc_ci="9999999", estado=True, id_lista=lista, id_tipo_cliente=tipo
        )
    ...
    def test_perform_create_sin_nro_tarjeta(self, factory, user, hijo):
        ...
        if serializer.is_valid():
            ...
            try:
                view.perform_create(serializer)
                registro = RegistrosConsumoAlmuerzo.objects.filter(id_hijo=hijo, nro_tarjeta__isnull=True).first()
                assert registro is not None
    ...
    def test_perform_create_integracion_api_con_tarjeta(self, client_api, user, hijo):
        ...
        # Act
        response = client_api.post("/api/registros-consumo-almuerzo/", payload, format="json")
...
@pytest.mark.parametrize(
    "tiene_tarjeta,nro_tarjeta_valor",
    [
...
def test_perform_create_parametrico(factory, tiene_tarjeta, nro_tarjeta_valor):
    ...
    tipo = TiposCliente.objects.create(nombre_tipo="Padre", estado=True)
    cliente = Clientes.objects.create(
        nombres="Test", apellidos="Parametrico", ruc_ci="8888888", estado=True, id_lista=lista, id_tipo_cliente=tipo
    )
