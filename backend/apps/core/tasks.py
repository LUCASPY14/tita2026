...
def expirar_recargas_pendientes(self):
    ...
    try:
        ...
        recargas_a_expirar = CargasSaldo.objects.filter(
            estado__in=['pendiente', 'pendiente_validacion'],
            fecha_carga__lt=limite_expiracion
        ).select_for_update()
        ...
        for tarjeta in tarjetas:
            try:
                with transaction.atomic():
                    ...
                    suma_consumos = ConsumosTarjeta.objects.filter(
                        nro_tarjeta=tarjeta
                    ).aggregate(
...
def limpiar_cache_configuraciones():
    ...
    try:
        ...
        eliminados = CacheConfiguracion.objects.filter(
