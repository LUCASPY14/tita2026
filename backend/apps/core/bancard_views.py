...
Endpoints:
  POST /api/v1/bancard/iniciar/         → Inicia pago, devuelve URL de Bancard
  GET  /api/v1/bancard/retorno/         → Bancard redirige aquí tras el pago
  GET  /api/v1/bancard/estado/<spid>/   → Consulta estado de un pago (para el portal)
"""
...
def bancard_retorno(request):
    ...
    if resultado.get("status") == "success":
        ...
        if response_code == "00":
            try:
                ...
                logger.error("Error acreditando saldo shop=%s: %s", shop_process_id, exc)
                pago.estado = PagoBancard.Estado.ERROR
                pago.save(update_fields=["estado"])
                return HttpResponseRedirect(
                    f"{PORTAL_URL()}/portal/carga-saldo?estado=error&msg=acreditacion"
                )
    ...
    return HttpResponseRedirect(
        f"{PORTAL_URL()}/portal/carga-saldo?estado=rechazado&monto={pago.monto}"
    )
