    ...
    def handle(self, *args, **options):
        ...
        if reset:
            deleted, _ = PeriodicTask.objects.filter(
                name__in=[t['name'] for t in PERIODIC_TASKS]
            ).delete()
            self.stdout.write(
                self.style.WARNING(f'  Eliminadas {deleted} tareas existentes.')
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Total en DB: {PeriodicTask.objects.count()} tarea(s) periódica(s).'
            )
        )
