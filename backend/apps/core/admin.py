...
class TarjetasAdmin(admin.ModelAdmin):
    ...
    def alerta_saldo(self, obj):
        ...
        if obj.esta_en_alerta:
            return format_html(
                '<span style="color:#dc3545;">⚠️ Saldo bajo</span>'
            )
...
class CargasSaldoAdmin(admin.ModelAdmin):
    ...
    def cliente_info(self, obj):
        ...
        if obj.id_cliente_origen:
            return format_html(
                '{} {}',
                obj.id_cliente_origen.nombre,
                obj.id_cliente_origen.apellido
            )
...
class TransaccionesOnlineAdmin(admin.ModelAdmin):
    ...
    def monto_display(self, obj):
        """Monto formateado"""
        return format_html(
            '<strong style="color:#0d6efd;">₲{:,.2f}</strong>',
            obj.monto
        )
    monto_display.short_description = 'Monto'
...
class ConfiguracionSistemaAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Valor', {
            'fields': ('valor', 'valor_defecto', 'tipo')
        }),
        ('Validación', {
...
class LimitesTransaccionAdmin(admin.ModelAdmin):
    ...
    def monto_limite_display(self, obj):
        ...
        return format_html(
            '<strong style="color:#0d6efd;">₲{:,.2f}</strong>',
            obj.monto_maximo_sin_autorizacion
        )
...
class RegistroAutorizacionesAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Referencias', {
            'fields': ('id_venta', 'id_compra', 'id_ajuste'),
            'classes': ('collapse',)
        }),
        ('Auditoría', {
    ...
    def solicitante_info(self, obj):
        ...
        if obj.id_empleado_solicitante:
            return format_html(
                '{} {}',
                obj.id_empleado_solicitante.nombre,
                obj.id_empleado_solicitante.apellido
            )
        return '-'
