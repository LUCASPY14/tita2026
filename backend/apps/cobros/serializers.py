...
class PagosClientesSerializer(serializers.ModelSerializer):
    ...
    monto_aplicado = serializers.DecimalField(
        max_digits=12,
        decimal_places=2,
        read_only=True
    )
    monto_pendiente_aplicar = serializers.DecimalField(
        max_digits=12,
        decimal_places=2,
        read_only=True
    )
        ...
        for app in value:
            if 'id_venta' not in app or 'monto_aplicado' not in app:
                raise serializers.ValidationError(
                    "Cada aplicación debe tener 'id_venta' y 'monto_aplicado'"
                )
            ...
            if monto <= 0:
                raise serializers.ValidationError(
                    f"El monto aplicado debe ser positivo (venta {app['id_venta']})"
                )
            ...
            try:
                ...
                if venta.saldo_pendiente <= 0:
                    raise serializers.ValidationError(
                        f"La venta {app['id_venta']} no tiene saldo pendiente"
                    )
    ...
    def validate(self, attrs):
        ...
        if aplicaciones:
            ...
            total_aplicaciones = sum(
                Decimal(str(app['monto_aplicado'])) for app in aplicaciones
            )
            monto_total = attrs['monto_total']
            
            if total_aplicaciones > monto_total:
                raise serializers.ValidationError(
                    f"La suma de aplicaciones ({total_aplicaciones}) excede "
