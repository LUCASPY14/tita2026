...
class PagosClientesViewSet(viewsets.ModelViewSet):
    ...
    def facturas_pendientes(self, request):
        ...
        if not id_cliente:
            return Response({"detail": "Parámetro id_cliente es requerido"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        facturas = Ventas.objects.filter(id_cliente=cliente, saldo_pendiente__gt=0).order_by("fecha")
        ...
        return Response(
            {
                ...
                "resumen": {"cantidad_facturas": facturas.count(), "total_pendiente": total_pendiente},
            }
        )
    ...
    def registrar_pago(self, request):
        ...
        try:
            ...
            return Response({"detail": "Usuario no tiene empleado asociado"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not aplicaciones:
            ...
            aplicaciones = self._aplicar_automatico(data["id_cliente"], data["monto_total"])

        # Crear aplicaciones y actualizar saldos
        for app_data in aplicaciones:
            venta = Ventas.objects.select_for_update().get(id_venta=app_data["id_venta"])
            monto_aplicado = Decimal(str(app_data["monto_aplicado"]))

            # Crear aplicación
            AplicacionPagosClientes.objects.create(id_pago_cliente=pago, id_venta=venta, monto_aplicado=monto_aplicado)
    ...
    def _aplicar_automatico(self, id_cliente, monto_total):
        ...
        facturas = Ventas.objects.filter(id_cliente_id=id_cliente, saldo_pendiente__gt=0).order_by("fecha")
        ...
        for factura in facturas:
            ...
            aplicaciones.append({"id_venta": factura.id_venta, "monto_aplicado": float(monto_a_aplicar)})
    ...
    def generar_qr_sipap(self, request):
        ...
        if not id_cliente:
            return Response({"detail": "id_cliente es requerido"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if total_deuda <= 0:
            return Response({"detail": "Cliente no tiene deuda pendiente"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            qr_data = sipap.generar_qr_dinamico(
                id_cobro=pago_pendiente.id_pago_cliente, monto=monto, descripcion=descripcion, id_cliente=id_cliente
            )
            ...
            return Response(
                {"detail": f"Error al generar QR SIPAP: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
