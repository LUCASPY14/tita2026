...
class NotificacionServiceSaldoBajoTest(TestCase):
    ...
    @patch('apps.notificaciones.services.email_service.EmailService.enviar_alerta_saldo_bajo', return_value={'success': True})
    ...
    def test_notificacion_exitosa_con_email_y_sms(
        self, mock_select, mock_create, mock_sms, mock_email
    ):
        from apps.notificaciones.services import NotificacionService
...
class NotificacionServiceSaldoBajoEmailErrorTest(TestCase):
    ...
    @patch('apps.notificaciones.services.email_service.EmailService.enviar_alerta_saldo_bajo',
           side_effect=Exception("Email error"))
    @patch('apps.notificaciones.services.sms_service.SMSService.enviar_sms',
           side_effect=Exception("SMS error"))
    @patch('apps.notificaciones.services.NotificacionesSaldo.objects.create')
    @patch('apps.notificaciones.services.Tarjetas.objects.select_related')
    def test_email_y_sms_exception_pasan_silenciosamente(
        self, mock_select, mock_create, mock_sms, mock_email
    ):
        from apps.notificaciones.services import NotificacionService
...
class NotificacionServiceRecargaCompletadaTest(TestCase):
    ...
    @patch('apps.notificaciones.services.UsuariosPortal.objects.get')
    @patch('apps.core.models.CargasSaldo.objects.select_related')
    def test_recarga_completada_crea_notificacion_portal(
        self, mock_select, mock_usuario_get, mock_portal_create
    ):
        from apps.notificaciones.services import NotificacionService
        ...
        result = NotificacionService.enviar_notificacion_recarga(
            id_recarga=1, id_usuario_portal=5
        )
        self.assertTrue(result['success'])
    ...
    @patch('apps.notificaciones.services.UsuariosPortal.objects.get')
    @patch('apps.core.models.CargasSaldo.objects.select_related')
    def test_recarga_sin_id_usuario_portal_no_existe(
        self, mock_select, mock_usuario_get
    ):
        from apps.notificaciones.services import NotificacionService
        from apps.usuarios.models import UsuariosPortal
...
class NotificacionServiceConsumoExitosoTest(TestCase):
    ...
    @patch('apps.notificaciones.services.UsuariosPortal.objects.get')
    @patch('apps.core.models.ConsumosTarjeta.objects.select_related')
    def test_consumo_exitoso_con_usuario_portal(
        self, mock_select, mock_usuario_get, mock_portal_create
    ):
        from apps.notificaciones.services import NotificacionService
        import django.utils.timezone as tz_module
        ...
        result = NotificacionService.enviar_notificacion_consumo(
            id_consumo=1, id_usuario_portal=5
        )
        self.assertTrue(result['success'])
...
class NotificacionServiceAlertasConTarjetasTest(TestCase):
    ...
    def test_con_tarjeta_sin_alerta_reciente_genera_alerta(
        self, mock_tarjetas_filter, mock_notif_filter, mock_enviar
    ):
        from apps.notificaciones.services import NotificacionService
    ...
    @patch('apps.notificaciones.services.NotificacionesSaldo.objects.filter')
    @patch('apps.notificaciones.services.Tarjetas.objects.filter')
    def test_con_alerta_reciente_salta(
        self, mock_tarjetas_filter, mock_notif_filter, mock_enviar
    ):
        from apps.notificaciones.services import NotificacionService
    ...
    @patch('apps.notificaciones.services.NotificacionesSaldo.objects.filter')
    @patch('apps.notificaciones.services.Tarjetas.objects.filter')
    def test_error_al_enviar_registra_en_errores(
        self, mock_tarjetas_filter, mock_notif_filter, mock_enviar
    ):
        from apps.notificaciones.services import NotificacionService
...
class NotificacionServiceMarcarLeidaTest(TestCase):
    ...
    def test_marcar_portal_leida(self, mock_get):
        ...
        result = NotificacionService.marcar_notificacion_leida(
            id_notificacion=1, tipo='portal'
        )
        self.assertTrue(result['success'])
...
class NotificacionServiceSaldoBajoRamasFaltantesTest(TestCase):
    ...
    @patch('apps.notificaciones.services.NotificacionesSaldo.objects.create')
    @patch('apps.notificaciones.services.Tarjetas.objects.select_related')
    def test_email_destinatario_provisto_salta_asignacion_cliente(
        self, mock_select, mock_create, mock_email
    ):
    ...
    @patch('apps.notificaciones.services.NotificacionesSaldo.objects.create')
    @patch('apps.notificaciones.services.Tarjetas.objects.select_related')
    def test_sin_email_no_enviar_email(
        self, mock_select, mock_create
    ):
    ...
    @patch('apps.notificaciones.services.NotificacionesSaldo.objects.create',
           side_effect=Exception("DB create error"))
    @patch('apps.notificaciones.services.Tarjetas.objects.select_related')
    def test_excepcion_inesperada_eleva_validation_error(
        self, mock_select, mock_create
    ):
...
class NotificacionServiceRecargaSinIdEncontradoTest(TestCase):
    ...
    @patch('apps.notificaciones.services.UsuariosPortal.objects.get')
    @patch('apps.core.models.CargasSaldo.objects.select_related')
    def test_sin_id_portal_pero_encontrado_en_db(
        self, mock_select, mock_usuario_get, mock_portal_create
    ):
...
class NotificacionServiceConsumoPortalEncontradoTest(TestCase):
    ...
    @patch('apps.notificaciones.services.UsuariosPortal.objects.get')
    @patch('apps.core.models.ConsumosTarjeta.objects.select_related')
    def test_consumo_excepcion_inesperada_raises(
        self, mock_select, mock_usuario_get, mock_portal_create
    ):
...
class NotificacionServiceAlertasRamasFaltantesTest(TestCase):
    ...
    @patch('apps.notificaciones.services.NotificacionesSaldo.objects.filter')
    @patch('apps.notificaciones.services.Tarjetas.objects.filter')
    def test_alerta_generada_sin_envio_email_ni_sms(
        self, mock_tarjetas_filter, mock_notif_filter, mock_enviar
    ):
