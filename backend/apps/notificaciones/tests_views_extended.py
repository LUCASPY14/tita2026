...
class NotificacionesSaldoFiltroTest(APITestCase):
    ...
    def setUp(self):
        ...
        hijo = Hijos.objects.create(nombre="H", apellido="Ijo", estado=True, id_cliente_responsable=cliente)
...
class AlertasSistemaFechaFiltroTest(APITestCase):
    ...
    def setUp(self):
        auth_user = User.objects.create_user(
            username="alerta_fecha_user", email="alerta_fecha@test.com", password="pass123", is_staff=True
        )
        self.client = APIClient()
        self.client.force_authenticate(user=auth_user)
...
class AlertasSistemaResolverConEmpleadoTest(APITestCase):
    ...
    def setUp(self):
        auth_user = User.objects.create_user(
            username="alerta_emp_user", email="alerta_emp@test.com", password="pass123", is_staff=True
        )
        self.client = APIClient()
        self.client.force_authenticate(user=auth_user)
...
class AlertasSistemaResolverConIdEmpleadoTest(APITestCase):
    ...
    def setUp(self):
        auth_user = User.objects.create_user(
            username="resolver_emp2", email="resolver_emp2@test.com", password="pass123", is_staff=True
        )
        self.client = APIClient()
        self.client.force_authenticate(user=auth_user)
        self.alerta = AlertasSistema.objects.create(
