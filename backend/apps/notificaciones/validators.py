...
def validar_tipo_notificacion_portal(value):
    ...
    tipos_validos = [
        'alerta', 'recordatorio', 'venta', 'compra', 'inventario',
    ...
    if value.lower() not in tipos_validos:
        ...
        raise ValidationError(
            f"Tipo de notificación no válido. Tipos permitidos: {tipos_str}"
        )
...
def validar_destino_notificacion(value):
    ...
    if value not in destinos_validos:
        raise ValidationError(
            f"Destino no válido. Debe ser uno de: {', '.join(destinos_validos)}"
        )
...
def validar_tipo_preferencia_notificacion(value):
    ...
    tipos_validos = [
        'compras', 'ventas', 'inventario', 'promociones',
    ...
    if value.lower() not in tipos_validos:
        tipos_str = ', '.join(tipos_validos)
        raise ValidationError(
            f"Tipo de preferencia no válido. Tipos permitidos: {tipos_str}"
        )
...
def validar_estado_email(value):
    ...
    estados_validos = [
        'Pendiente', 'Enviado', 'Entregado', 'Fallido', 'Rebotado',
        'Abierto', 'Marcado_Spam'
    ]
...
def validar_mensaje_sms(value):
    ...
    if len(value) > 160:
        raise ValidationError(
            "El mensaje no puede exceder 160 caracteres (límite SMS)."
        )
...
def validar_estado_sms(value):
    ...
    estados_validos = [
        'Pendiente', 'Enviado', 'Entregado', 'Fallido', 'Rechazado'
    ]
...
def validar_categoria_template(value):
    ...
    categorias_validas = [
        'Ventas', 'Compras', 'Inventario', 'Promociones',
...
def validar_tipo_campana(value):
    ...
    if value not in tipos_validos:
        raise ValidationError(
            f"Tipo no válido. Debe ser uno de: {', '.join(tipos_validos)}"
        )
...
def validar_estado_campana(value):
    ...
    estados_validos = [
        'Borrador', 'Programada', 'Enviando', 'Enviada', 'Cancelada', 'Fallida'
    ]
...
def validar_tipo_alerta(value):
    ...
    tipos_validos = [
        'Inventario', 'Ventas', 'Compras', 'Saldo', 'Sistema', 'Seguridad'
    ]
...
def validar_tipo_anomalia(value):
    ...
    tipos_validos = [
        'acceso_inusual', 'intentos_fallidos', 'cambio_horario',
...
def validar_dia_semana_restriccion(value):
    ...
    dias_validos = [
        'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo', 'Todos'
    ]
