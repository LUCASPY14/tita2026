...
class NotificacionService:
    ...
    def enviar_notificacion_recarga(
        ...
        try:
            ...
            if not id_usuario_portal:
                try:
                    usuario_portal = UsuariosPortal.objects.get(
                    ...
                    logger.warning(
                        f"Cliente {hijo.id_cliente.id_cliente} no tiene usuario portal"
                    )
                    return {
                        'success': False,
        ...
        try:
            ...
            if not id_usuario_portal:
                try:
                    ...
                    return {
                        'success': False,
                        'error': 'Cliente no tiene usuario portal'
                    }
            else:
                id_usuario_portal_obj = UsuariosPortal.objects.get(
                    id_usuario_portal=id_usuario_portal
    ...
    def generar_alertas_automaticas() -> Dict:
        ...
        try:
            ...
            tarjetas_bajo = Tarjetas.objects.filter(
                estado='Activa',
                saldo_actual__lte=models.F('saldo_alerta')
            ...
            for tarjeta in tarjetas_bajo:
                ...
                try:
                    ...
                    errores.append(
                        f"Tarjeta {tarjeta.numero_tarjeta}: {str(e)}"
                    )
    ...
    def marcar_notificacion_leida(id_notificacion: int, tipo: str = 'portal') -> Dict:
        ...
        try:
            if tipo == 'portal':
                notificacion = NotificacionesPortal.objects.get(
                    id_notificacion=id_notificacion
                )
            else:
                notificacion = NotificacionesSaldo.objects.get(
                    id_notificacion=id_notificacion
                )
