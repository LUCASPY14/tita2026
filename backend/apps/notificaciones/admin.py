...
class NotificacionesPortalAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Información", {"fields": ("id_notificacion", "tipo", "id_usuario_portal")}),
        ("Contenido", {"fields": ("titulo", "mensaje")}),
        ("Estado", {"fields": ("leida", "fecha_envio", "fecha_lectura", "creado_en")}),
    )
...
class NotificacionesSaldoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Información", {"fields": ("id_notificacion", "tipo_notificacion", "nro_tarjeta")}),
        ("Saldo", {"fields": ("saldo_actual", "mensaje")}),
        (
            "Estado de Envío",
            {
                "fields": (
...
class SolicitudesNotificacionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Información", {"fields": ("id_solicitud", "id_cliente", "nro_tarjeta")}),
        ("Configuración Alerta", {"fields": ("saldo_alerta", "mensaje", "destino")}),
        ("Estado", {"fields": ("estado", "fecha_solicitud", "fecha_envio")}),
    )
    ...
    def estado_badge(self, obj):
        colores = {"Pendiente": "orange", "Enviada": "green", "Cancelada": "red"}
        color = colores.get(obj.estado, "gray")
        return format_html(
            '<span style="background-color: {}; color: white; padding: 3px 10px; border-radius: 3px;">{}</span>',
            color,
            obj.estado or "Sin Estado",
        )
...
class PreferenciasNotificacionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Información", {"fields": ("id_preferencia", "id_usuario_portal", "tipo_notificacion")}),
        ("Preferencias", {"fields": ("email_activo", "push_activo")}),
...
class EmailsEnviadosAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Destinatario", {"fields": ("email_destinatario", "nombre_destinatario", "id_cliente")}),
        ("Contenido", {"fields": ("asunto", "cuerpo", "id_template")}),
        ("Estado", {"fields": ("estado", "intentos", "mensaje_error")}),
        ("Fechas", {"fields": ("fecha_envio", "fecha_entrega", "fecha_apertura")}),
        ("Registro", {"fields": ("enviado_por",)}),
    )
...
class SmsEnviadosAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Destinatario", {"fields": ("telefono", "id_cliente")}),
        ("Mensaje", {"fields": ("mensaje", "id_template")}),
        ("Estado", {"fields": ("estado", "costo")}),
        ("Fechas", {"fields": ("fecha_envio", "fecha_entrega")}),
        ("Registro", {"fields": ("enviado_por",)}),
    )
...
class CampanasComunicacionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Información", {"fields": ("id_campana", "nombre", "descripcion", "tipo")}),
        ("Segmentación", {"fields": ("segmentacion",)}),
        ("Plantillas", {"fields": ("id_email_template", "id_sms_template")}),
        ("Programación", {"fields": ("fecha_programada", "fecha_enviada", "estado")}),
        ("Estadísticas", {"fields": ("total_destinatarios", "total_enviados", "total_entregados")}),
        ("Registro", {"fields": ("created_at", "created_by")}),
    )

    def tipo_badge(self, obj):
        colores = {"Email": "blue", "SMS": "green", "Mixta": "purple", "Push": "orange"}
...
class AlertasSistemaAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Información", {"fields": ("id_alerta", "tipo", "mensaje")}),
        ("Estado", {"fields": ("estado", "fecha_creacion", "fecha_leida")}),
        ("Resolución", {"fields": ("id_empleado_resuelve", "fecha_resolucion", "observaciones")}),
    )
    ...
    def estado_badge(self, obj):
        colores = {"Pendiente": "orange", "Resuelta": "green", "Ignorada": "gray"}
        color = colores.get(obj.estado, "gray")
        return format_html(
            '<span style="background-color: {}; color: white; padding: 3px 10px; border-radius: 3px;">{}</span>',
            color,
...
class HistorialAlertasAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Información", {"fields": ("id_historial", "id_alerta", "fecha_disparada")}),
        ("Mensaje", {"fields": ("mensaje", "datos_contexto")}),
        ("Resolución", {"fields": ("resuelto", "resuelto_por", "fecha_resolucion")}),
    )
...
class AnomaliasDetectadasAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Información", {"fields": ("id_anomalia", "usuario", "fecha_deteccion")}),
        ("Anomalía", {"fields": ("tipo_anomalia", "nivel_riesgo", "descripcion", "ip_address")}),
        ("Estado", {"fields": ("notificado",)}),
    )
