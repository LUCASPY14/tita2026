...
from apps.ventas.models import Ventas, PagosVenta
from apps.core.models import Tarjetas, MediosPago
from apps.contabilidad.models import TarifasComision, MovimientosCaja
...
class ComisionesBancardTest(TestCase):
    ...
    def setUp(self):
        ...
        self.cliente = Clientes.objects.create(
            nombres='Pedro', apellidos='Martínez', ruc_ci='12345678',
            limite_credito=Decimal('1000.00'), activo=True,
            id_lista=self.lista, id_tipo_cliente=self.tipo_cliente
        )
        
        # Crear hijo
        self.hijo = Hijos.objects.create(
            nombre='Juan', apellido='Martínez', grado='5to',
        ...
        self.cajero = Empleados.objects.create(
            ...
            usuario='cajero_test', contrasena_hash='hash789',
    ...
    def test_multiples_valores_comision(self):
        ...
        for monto, comision_esperada in casos_prueba:
            ...
            self.assertEqual(comision, comision_esperada,
...
class IntegracionComisionesTest(TestCase):
    ...
    def setUp(self):
        ...
        self.tipo_cliente = TiposCliente.objects.create(nombre_tipo='Regular', activo=True)
        self.cliente = Clientes.objects.create(
            nombres='Ana', apellidos='López', ruc_ci='87654321',
        ...
        self.empleado = Empleados.objects.create(
            ...
            usuario='gerente_test', contrasena_hash='hash321',
            fecha_ingreso=timezone.now(),
            activo=True, id_rol=self.rol
        )
        self.debito, _ = MediosPago.objects.get_or_create(
    ...
    def test_venta_pago_directo_con_comision(self):
        ...
        # Registrar pago con comisión
        viewset = VentasViewSet()
        pago = viewset._registrar_pago_con_comision(
            venta, 
            self.debito, 
