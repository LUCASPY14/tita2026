...
def validar_monto_positivo(valor):
    ...
    if not isinstance(valor, (Decimal, float, int)):
        raise ValidationError(
            f'El valor debe ser numérico, recibido: {type(valor).__name__}'
        )
    ...
    if valor_decimal <= 0:
        raise ValidationError(
            f'El monto debe ser mayor a cero. Valor recibido: {valor_decimal}'
        )
...
def validar_codigo_promocion(codigo):
    ...
    if len(codigo) < 3 or len(codigo) > 20:
        raise ValidationError(
            f'El código debe tener entre 3 y 20 caracteres. '
            f'Longitud actual: {len(codigo)}'
        )
...
def validar_porcentaje_descuento(porcentaje):
    ...
    if porcentaje_decimal < 0:
        raise ValidationError(
            f'El porcentaje no puede ser negativo. Valor: {porcentaje_decimal}'
        )
...
def validar_estado_pago(estado):
    ...
    if estado not in estados_validos:
        raise ValidationError(
            f'Estado de pago inválido: {estado}. '
            f'Estados válidos: {", ".join(estados_validos)}'
        )
...
def validar_cantidad_producto(cantidad):
    ...
    if cantidad_decimal <= 0:
        raise ValidationError(
            f'La cantidad debe ser mayor a cero. Valor: {cantidad_decimal}'
        )
...
def validar_dias_semana(dias):
    ...
    for dia in dias:
        ...
        if dia < 0 or dia > 6:
            raise ValidationError(
                f'Los días deben estar entre 0 (Lunes) y 6 (Domingo). '
...
def validar_numero_factura(numero):
    ...
    if not numero_str.isdigit():
        raise ValidationError(
            f'El número de factura solo puede contener dígitos. '
            f'Valor recibido: {numero_str}'
        )
...
def validar_motivo_credito(motivo, tipo_venta):
    ...
    if tipo_venta == 'Crédito':
        if not motivo or motivo.strip() == '':
            raise ValidationError(
                'Las ventas a crédito requieren un motivo'
            )
        
        if len(motivo) < 10:
            raise ValidationError(
                f'El motivo debe tener al menos 10 caracteres. '
                f'Longitud actual: {len(motivo)}'
            )
