...
class VentasModelsAdicionalesTest(TestCase):
    ...
    def test_str_productos_promocion(self):
        promo = Promociones.objects.create(
            nombre="Promo Prod Str",
            tipo_promocion="monto_fijo",
            valor_descuento=Decimal("1000"),
            fecha_inicio=timezone.now().date(),
            aplica_a="productos",
            min_cantidad=1,
            monto_minimo=Decimal("0"),
