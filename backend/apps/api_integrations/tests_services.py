...
class BancardServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.proveedor = ProveedoresApi.objects.create(
            ...
            config_auth={"public_key": "test_public_key_123", "private_key": "test_private_key_456"},
            timeout=30,
            max_reintentos=3,
        ...
        self.recarga = CargasSaldo.objects.create(
            monto=Decimal("50000.00"), estado="pendiente", metodo_pago="bancard", fecha_creacion=timezone.now()
        )
    ...
    def test_validar_webhook_signature_valid(self):
        ...
        signature = hmac.new(service.private_key.encode("utf-8"), mensaje.encode("utf-8"), hashlib.sha256).hexdigest()
    ...
    def test_validar_webhook_signature_tampering(self):
        ...
        valid_signature = hmac.new(
            service.private_key.encode("utf-8"), mensaje.encode("utf-8"), hashlib.sha256
        ).hexdigest()
    ...
    def test_log_api_call_exception_handling(self):
        ...
        with patch("apps.api_integrations.models.LogsLlamadasApi.objects.create") as mock_create:
            ...
            try:
                service._log_api_call(metodo="GET", url="https://api.test.com/status", exitoso=True)
            except Exception:
    ...
    def test_iniciar_transaccion_with_buyer_info(self, mock_post):
        ...
        mock_response = Mock()
        mock_response.json.return_value = {"status": "success", "process_id": "PROC-WITH-BUYER"}
        mock_response.status_code = 200
    ...
    def test_iniciar_transaccion_bancard_error_response(self, mock_post):
        ...
        mock_response.json.return_value = {
            "status": "error",
            "messages": [{"key": "InvalidAmount", "dsc": "El monto debe ser mayor a 0"}],
        }
    ...
    def test_iniciar_transaccion_monto_formatting(self, mock_post):
        ...
        montos_test = [Decimal("1234.56"), Decimal("1000.00"), Decimal("999.99"), Decimal("50000")]
