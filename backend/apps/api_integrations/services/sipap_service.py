...
class SIPAPService:
    ...
    def __init__(self, ambiente: str = None, banco_agregador: str = None):
        ...
        if not self.base_url:
            raise ValueError(
                f"URL no configurada para ambiente={self.ambiente}, banco={self.banco_agregador}"
            )
    ...
    def generar_qr_dinamico(
        ...
        try:
            ...
            if not response.ok:
                raise Exception(
                    f"Error API SIPAP: {response.status_code} - {response.text}"
                )
    ...
    def procesar_webhook(self, payload: Dict[str, Any], firma: str, ip_origen: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if estado != 'aprobado':
                return {
                    'success': False,
                    'mensaje': f"Pago no aprobado - Estado: {estado}",
                    'txn_id': txn_id
                }
            ...
            if PagosClientes.objects.filter(referencia=txn_id).exists():
                return {
                    'success': True,
                    'mensaje': 'Pago ya procesado anteriormente (duplicado)',
                    'txn_id': txn_id
                }
            ...
            with transaction.atomic():
                ...
                medio_pago_sipap, _ = MediosPago.objects.get_or_create(
                    descripcion='SIPAP QR',
                    defaults={'activo': True, 'requiere_autorizacion': False}
                )
    ...
    def _validar_firma_webhook(self, payload: Dict[str, Any], firma: str) -> bool:
        ...
        try:
            ...
            public_key = serialization.load_pem_public_key(
                self.banco_public_key.encode('utf-8'),
