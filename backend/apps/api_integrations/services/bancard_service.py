...
class BancardService:
    ...
    def _validar_webhook_signature(
        ...
        firma_calculada = hmac.new(
            self.private_key.encode("utf-8"), mensaje.encode("utf-8"), hashlib.sha256
        ).hexdigest()
    ...
    def procesar_webhook(
        ...
        try:
            ...
            if not self._validar_webhook_signature(shop_process_id, operation, signature):
                return {"success": False, "error": "Firma inválida. Posible intento de fraude."}
            ...
            try:
                ...
                return {"success": False, "error": f"shop_process_id inválido: {shop_process_id}"}

            # 3. Obtener recarga
            try:
                recarga = CargasSaldo.objects.select_for_update().get(id_carga=recarga_id)
            except CargasSaldo.DoesNotExist:
                return {"success": False, "error": f"Recarga {recarga_id} no encontrada"}
            ...
            return {"success": False, "error": f"Error al procesar webhook: {str(e)}"}
    ...
    def confirmar_transaccion(self, shop_process_id: str) -> Dict[str, Any]:
        ...
        payload = {"public_key": self.public_key, "operation": {"shop_process_id": shop_process_id}}
