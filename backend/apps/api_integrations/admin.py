...
class ProveedoresApiAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'id_proveedor', 'nombre', 'tipo_servicio_badge', 'version',
        'tipo_auth_badge', 'timeout', 'max_reintentos', 'activo_badge', 'created_at'
    )
...
class EndpointsApiAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'id_endpoint', 'nombre', 'metodo_badge', 'path', 'id_proveedor',
        'requiere_auth_badge', 'cache_segundos', 'activo_badge'
    )
    ...
    fieldsets = (
        ...
        ('🌐 Configuración HTTP', {
            'fields': ('path', 'metodo', 'headers', 'parametros')
        }),
        ('📋 Esquemas', {
            'fields': ('schema_request', 'schema_response')
        }),
        ('⚙️ Opciones', {
            'fields': ('requiere_auth', 'cache_segundos', 'activo')
        }),
    )
    ...
    def requiere_auth_badge(self, obj):
        ...
        return format_html(
            '<span style="color: #6c757d;">🔓 Público</span>'
        )
...
class LogsLlamadasApiAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'id_log', 'timestamp', 'metodo_badge', 'url_corta',
        'status_badge', 'tiempo_ms', 'exitoso_badge', 'intento', 'ip_origen'
    )
    ...
    fieldsets = (
        ...
        ('📤 Request', {
            'fields': ('metodo', 'url', 'headers_req', 'payload_req', 'bytes_sent')
        }),
        ('📥 Response', {
            'fields': ('status_code', 'headers_res', 'payload_res', 'bytes_received')
        }),
        ('⏱️ Rendimiento', {
            'fields': ('tiempo_ms', 'exitoso', 'intento')
        }),
        ('🔍 Detalles', {
            'fields': ('error_msg', 'ip_origen', 'contexto')
        }),
    )
...
class CredencialesApiAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'id_credencial', 'id_proveedor', 'ambiente_badge',
        'tiene_api_key', 'tiene_secret', 'tiene_token',
        'fecha_expiracion', 'activo_badge', 'updated_at'
    )
    ...
    fieldsets = (
        ('🔑 Información de Credencial', {
            'fields': ('id_credencial', 'id_proveedor', 'ambiente')
        }),
    ...
    def ambiente_badge(self, obj):
        ...
        return format_html(
            '<span style="background-color: {}; color: white; padding: 3px 8px; '
            'border-radius: 3px; font-weight: bold; font-size: 11px;">{} {}</span>',
            color, icono, obj.ambiente.upper()
        )
...
class LogsWebhooksAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'id_log', 'timestamp', 'evento_tipo', 'ip_origen',
        'verificacion_badge', 'procesado_badge', 'tiempo_proc_ms', 'id_webhook'
    )
    ...
    fieldsets = (
        ...
        ('📬 Datos Recibidos', {
            'fields': ('headers', 'payload', 'ip_origen', 'user_agent')
        }),
        ('✅ Procesamiento', {
    ...
    def procesado_badge(self, obj):
        if obj.procesado_ok:
            return format_html(
                '<span style="color: green; font-weight: bold;">✓ Procesado</span>'
            )
        return format_html(
            '<span style="color: red; font-weight: bold;">✗ Error</span>'
        )
...
class WebhookEndpointsAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'id_webhook', 'nombre', 'path', 'id_proveedor',
        'requiere_verificacion_badge', 'eventos_count', 'activo_badge', 'created_at'
    )
    ...
    fieldsets = (
        ...
        ('🌐 Configuración', {
            'fields': ('path', 'eventos', 'handler_func')
        }),
        ('🔒 Seguridad', {
    ...
    def requiere_verificacion_badge(self, obj):
        ...
        return format_html(
            '<span style="color: #6c757d;">🔓 Sin Verificación</span>'
        )
