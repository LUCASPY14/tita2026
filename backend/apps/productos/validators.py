...
def validar_stock_minimo(stock_minimo):
    ...
    if stock > 100000:
        raise ValidationError(
            f'El stock mínimo no puede exceder 100,000 unidades. '
            f'Recibido: {stock}.'
        )
...
def validar_precio_positivo(precio):
    ...
    if precio_decimal > 100000000:
        raise ValidationError(
            f'El precio no puede exceder ₲100,000,000. '
            f'Recibido: ₲{precio_decimal:,.2f}.'
        )
...
def validar_unicidad_precio_lista(id_producto, id_lista, id_precio=None):
    ...
    query = PreciosPorLista.objects.filter(
...
def validar_cambio_precio_historico(precio_anterior, precio_nuevo):
    ...
    if anterior == nuevo:
        raise ValidationError(
            'El precio anterior y el nuevo son iguales. '
            'No hay cambio para registrar.'
        )
