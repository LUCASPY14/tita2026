...
def validar_razon_social(razon_social):
    ...
    if len(razon_social) > 255:
        raise ValidationError(
            "La razón social no puede exceder 255 caracteres"
        )
...
def validar_numero_factura(numero_factura):
    ...
    if not (re.match(formato_paraguayo, numero_factura) or 
            re.match(formato_simple, numero_factura) or
            len(numero_factura) >= 5):
        raise ValidationError(
...
def validar_saldo_compra(saldo_pendiente, monto_total):
    ...
    if saldo < 0:
        raise ValidationError(
            f"El saldo pendiente no puede ser negativo. Valor: ₲{saldo:,.0f}"
        )
...
def validar_cantidad_compra(cantidad):
    ...
    if cantidad_decimal <= 0:
        raise ValidationError(
            f"La cantidad debe ser positiva. Valor recibido: {cantidad_decimal}"
        )
...
def validar_costo_unitario(costo_unitario):
    ...
    if costo <= 0:
        raise ValidationError(
            f"El costo unitario debe ser positivo. Valor recibido: ₲{costo:,.0f}"
        )
...
def validar_monto_pago(monto_pago):
    ...
    if monto <= 0:
        raise ValidationError(
            f"El monto de pago debe ser positivo. Valor: ₲{monto:,.0f}"
        )
...
def validar_aplicacion_pago(monto_aplicado, saldo_compra):
    ...
    if aplicado <= 0:
        raise ValidationError(
            f"El monto a aplicar debe ser positivo. Valor: ₲{aplicado:,.0f}"
        )
...
def validar_motivo_nota_credito(motivo):
    ...
    if len(motivo) < 10:
        raise ValidationError(
            "El motivo de la nota de crédito debe tener al menos 10 caracteres"
        )
