...
class CompraService:
    ...
    def obtener_cuenta_corriente_proveedor(id_proveedor: int) -> Dict:
        ...
        total_pagado = compras.aggregate(
            pagado=Sum('monto_total') - Sum('saldo_pendiente')
        )['pagado'] or Decimal('0.00')
        
        # Saldo pendiente total
        saldo_pendiente = compras.aggregate(
        ...
        for compra in compras.filter(saldo_pendiente__gt=0):
            compras_pendientes.append({
                ...
                'dias_vencimiento': (timezone.now().date() - compra.fecha_compra).days if compra.fecha_compra else None
            })
