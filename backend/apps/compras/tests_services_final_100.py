...
class ValidarCompraStringConversionTest(TestCase):
    ...
    def setUp(self):
        ...
        self.impuesto = Impuestos.objects.create(
            nombre_impuesto="IVA 10%", porcentaje=Decimal("10.00"), vigente_desde=timezone.now().date(), estado=True
        )
...
class ConfirmarCompraSinEmpleadosTest(TestCase):
    ...
    def setUp(self):
        ...
        self.proveedor = Proveedores.objects.create(
            razon_social="Proveedor Test", ruc="12345678-9", fecha_registro=timezone.now(), estado=True
        )
    ...
    def test_confirmar_compra_sin_empleados_registrados(self):
        ...
        resultado = CompraService.confirmar_compra(id_compra=self.compra.id_compra, empleado=None)
    ...
    def test_confirmar_compra_con_empleado_valido(self):
        ...
        rol = Roles.objects.create(nombre_rol="Gerente", descripcion="Rol de gerente", estado=True)
        ...
        resultado = CompraService.confirmar_compra(id_compra=self.compra.id_compra, empleado=empleado)
...
class CalcularTotalesIvaExentoTest(TestCase):
    ...
    def test_calcular_totales_con_iva_exento(self):
        ...
        detalles = [{"id_producto": self.producto_exento.id_producto, "cantidad": 10, "precio_unitario": "2000.00"}]
    ...
    def test_calcular_totales_con_iva_diferente(self):
        ...
        impuesto_21 = Impuestos.objects.create(
            nombre_impuesto="IVA 21%", porcentaje=Decimal("21.00"), vigente_desde=timezone.now().date(), estado=True
        )
        ...
        detalles = [{"id_producto": producto_21.id_producto, "cantidad": 5, "precio_unitario": "1000.00"}]
