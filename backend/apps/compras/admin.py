...
class ProveedoresAdmin(admin.ModelAdmin):
    list_display = ['id_proveedor', 'razon_social', 'ruc_display', 'telefono', 'email', 'ciudad', 'estado_badge', 'fecha_registro']
    ...
    fieldsets = (
        ('Información Básica', {
            'fields': ('razon_social', 'ruc', 'activo')
        }),
        ('Datos de Contacto', {
...
class ComprasAdmin(admin.ModelAdmin):
    list_display = ['id_compra', 'nro_factura_display', 'proveedor_nombre', 'fecha', 'monto_display', 'saldo_display', 'estado_badge']
    ...
    def estado_badge(self, obj):
        ...
        return format_html(
            ...
            bg_color, text_color, obj.estado_pago.upper()
        )
    estado_badge.short_description = 'Estado'
...
class DetallesCompraAdmin(admin.ModelAdmin):
    list_display = ['id_detalle', 'compra_info', 'producto_descripcion', 'cantidad', 'costo_display', 'subtotal_display', 'iva_display']
    ...
    def compra_info(self, obj):
        ...
        return format_html(
            'Compra #{} - {}',
            obj.id_compra_id,
            obj.id_compra.nro_factura or 'S/F'
        )
...
class PagosProveedoresAdmin(admin.ModelAdmin):
    list_display = ['id_pago_proveedor', 'medio_pago_nombre', 'fecha_creacion', 'monto_total_aplicado']
    ...
    def monto_total_aplicado(self, obj):
        ...
        total = AplicacionPagosCompras.objects.filter(
            id_pago_proveedor=obj
...
class NotasCreditoProveedorAdmin(admin.ModelAdmin):
    list_display = ['id_nota_proveedor', 'nro_factura_display', 'proveedor_nombre', 'fecha', 'monto_display', 'estado_badge', 'fecha_creacion']
    ...
    def estado_badge(self, obj):
        ...
        return format_html(
            '<span style="background-color: {}; color: {}; padding: 3px 10px; border-radius: 3px; font-weight: bold;">{}</span>',
            bg_color, text_color, obj.estado.upper()
        )
    estado_badge.short_description = 'Estado'
...
class DetallesNotaCreditoProveedorAdmin(admin.ModelAdmin):
    list_display = ['id_detalle_nc_proveedor', 'nota_info', 'producto_descripcion', 'cantidad', 'precio_display', 'subtotal_display']
