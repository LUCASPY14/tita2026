...
CREATE TABLE core_movimientotarjeta (
    id_movimiento_tarjeta bigint GENERATED BY DEFAULT AS IDENTITY,
    ...
    -- La unicidad de 'id_movimiento_tarjeta' la garantiza la secuencia IDENTITY; nada hace FK a esta tabla.
    -- Nota: la columna consumo_id (y su FK a core_consumotarjeta, tabla inexistente
    -- en el esquema actual) era un vestigio de un modelo eliminado — se quitó de este script.
    ...
    CONSTRAINT "core_movimientotarjeta_carga_id_5f45c578_fk_core_cargasaldo_id"
        FOREIGN KEY (carga_id) REFERENCES core_cargasaldo(id_carga) DEFERRABLE INITIALLY DEFERRED,
    CONSTRAINT "core_movimientotarjeta_creado_por_id_b0ef7395_fk_usuarios_id"
        FOREIGN KEY (creado_por_id) REFERENCES usuarios(id_usuario) DEFERRABLE INITIALLY DEFERRED
...
CREATE INDEX idx_mov_tarj_tarjeta_id_desc ON core_movimientotarjeta (tarjeta_id, id_movimiento_tarjeta DESC);
...
SELECT setval(
    pg_get_serial_sequence('core_movimientotarjeta', 'id_movimiento_tarjeta'),
    COALESCE((SELECT MAX(id_movimiento_tarjeta) FROM core_movimientotarjeta), 1)
);
...
CREATE TABLE auditoria_operaciones (
    ...
    CONSTRAINT "auditoria_operaciones_usuario_id_861f2b37_fk_usuarios_id"
        FOREIGN KEY (usuario_id) REFERENCES usuarios(id_usuario) DEFERRABLE INITIALLY DEFERRED
